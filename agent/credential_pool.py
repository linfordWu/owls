...
class CredentialPool:
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("owls_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("owls_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other OWLS profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, OWLS PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_owls_oauth_credentials

        for source_name, creds in (
            ("owls_pkce", read_owls_oauth_credentials()),
            ("claude_code", read_claude_code_credentials()),
        ):
            if creds and creds.get("accessToken"):
                # Check if user explicitly removed this source
                try:
                    from owls_cli.auth import is_source_suppressed
        ...
        if state:
            ...
            # `owls auth add nous --label <name>`).  Fall back to the
        ...
        # OWLS auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from owls_cli.auth import resolve_qwen_runtime_credentials
        ...
        # Respect user suppression — `owls auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the OWLS auth store.  Without this gate the removal is instantly
        ...
        # OWLS owns its own Codex auth state — we do NOT auto-import from
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "owls_pkce"}
        )
    ]
