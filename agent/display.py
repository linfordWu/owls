...
_TOOLS_WITHOUT_PROGRESS_EMOJI = {"skills_list", "skill_view", "terminal"}
...
def get_progress_tool_emoji(tool_name: str, default: str = "⚡") -> str:
    """Get the emoji used while a tool is actively executing.

    Some tools intentionally suppress their execution emoji so the progress
    line stays text-only.
    """
    if tool_name in _TOOLS_WITHOUT_PROGRESS_EMOJI:
        return ""
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in OWLS' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    """Animated spinner with owl thinking markers for CLI feedback during tool execution."""
    ...
    OWL_THINKING = ["🦉💭"]
    KAWAII_WAITING = OWL_THINKING
    KAWAII_THINKING = OWL_THINKING
    ...
    def get_waiting_faces(cls) -> list:
        """Return waiting markers from the active skin, falling back to the owl marker."""
        try:
            skin = _get_skin()
            if skin:
    ...
    def get_thinking_faces(cls) -> list:
        """Return thinking markers from the active skin, falling back to the owl marker."""
        try:
            skin = _get_skin()
            if skin:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("OWLS_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def get_cute_tool_message(
    ...
    if tool_name == "terminal":
        return _wrap(f"┊ terminal   {_trunc(args.get('command', ''), 42)}  {dur}")
    if tool_name == "process":
        action = args.get("action", "?")
