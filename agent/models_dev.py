...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.owls/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: OWLS ↔ models.dev
# ---------------------------------------------------------------------------

# OWLS provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → OWLS (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_owls_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a OWLS provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal OWLS agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a OWLS provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve OWLS ID → models.dev ID
...
def get_model_info(
    ...
    Accepts OWLS or models.dev provider ID.  Tries exact match then
