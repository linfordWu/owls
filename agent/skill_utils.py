...
from owls_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``OWLS_PLATFORM`` or
            ``OWLS_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.owls/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.owls/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    owls = metadata.get("owls") or {}
    if not isinstance(owls, dict):
        owls = {}
    return {
        "fallback_for_toolsets": owls.get("fallback_for_toolsets", []),
        "requires_toolsets": owls.get("requires_toolsets", []),
        "fallback_for_tools": owls.get("fallback_for_tools", []),
        "requires_tools": owls.get("requires_tools", []),
    }
