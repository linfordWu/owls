"""Gateway configuration management for the Linux web/API backend."""
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_owls_home() / "sessions")
    ...
    def get_connected_platforms(self) -> List[Platform]:
        """Return enabled Linux web/API endpoints."""
        connected = []
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_owls_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.owls/config.yaml (primary user-facing config)
    3. ~/.owls/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_owls_home()
    gw_data: dict = {}
