...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-OWLS-Session-Id header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists owls as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to owls
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_owls_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
def _derive_chat_session_id(
    ...
    the same OWLS session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through owls's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``owls sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the owls-api-server default.
        """
    ...
    @staticmethod
    def _request_user_id(request: "web.Request") -> Optional[str]:
        user_id = (request.headers.get("X-OWLS-User-Id") or "").strip()
        if not user_id or re.search(r"[\r\n\x00]", user_id):
            return None
        return user_id[:128]
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "owls"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return owls as an available model."""
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to continue an existing session by passing X-OWLS-Session-Id.
        ...
        provided_session_id = request.headers.get("X-OWLS-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-OWLS-Session-Id rejected: "
            ...
            # conversation map to the same OWLS session.  The first user
        ...
        if stream:
            ...
            def _on_tool_progress(event_type, name, preview, args, **kwargs):
                ...
                ``event: owls.tool.progress`` line that compliant frontends
        ...
        return web.json_response(response_data, headers={"X-OWLS-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-OWLS-Session-Id"] = session_id
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: owls.tool.progress`` SSE event so
    ...
    def _fenced_text(text: str) -> str:
        """Return text in a Markdown fence without being broken by backticks."""
        fence = "```"
        while fence in text:
            fence += "`"
        return f"{fence}text\n{text.rstrip()}\n{fence}"
    ...
    @classmethod
    def _format_direct_shell_result(cls, result: Dict[str, Any]) -> str:
        """Format terminal_tool JSON into an assistant message for Web chat."""
        output = str(result.get("output") or "")
        error = str(result.get("error") or "")
        approval = str(result.get("approval") or "")
        exit_note = str(result.get("exit_code_meaning") or "")
        exit_code = result.get("exit_code")

        parts: List[str] = []
        if output:
            parts.append(cls._fenced_text(output))
        if error:
            parts.append(f"Error: {error}")
        if approval:
            parts.append(approval)
        if exit_note:
            parts.append(exit_note)
        if isinstance(exit_code, int) and exit_code != 0:
            parts.append(f"Exit code: {exit_code}")

        if parts:
            return "\n\n".join(parts)
        return "Command exited with code 0 and produced no output."

    @classmethod
    def _execute_direct_shell_command(cls, command: str, session_id: str) -> str:
        """Execute a direct Web shell command through the shared terminal tool."""
        ...
        return cls._format_direct_shell_result(result)
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        ephemeral_system_prompt = instructions
        try:
            from owls_cli.shell_detection import looks_like_direct_shell_command
            direct_shell_command = looks_like_direct_shell_command(user_message)
        except Exception as exc:
            logger.debug("[api_server] direct shell detection failed: %s", exc)
            direct_shell_command = False

        if direct_shell_command:
            command = user_message.strip()

            async def _run_direct_shell_and_close():
                try:
                    q.put_nowait({
                        "event": "run.started",
                        "run_id": run_id,
                    ...
                    def _run_sync():
                        return self._execute_direct_shell_command(command, session_id)

                    final_response = await asyncio.get_running_loop().run_in_executor(None, _run_sync)
                    q.put_nowait({
                        "event": "message.delta",
                        "run_id": run_id,
                        "timestamp": time.time(),
                    ...
                    logger.exception("[api_server] direct shell run %s failed", run_id)
                    try:
                        q.put_nowait({
                            "event": "run.failed",
                            "run_id": run_id,
            ...
            task = asyncio.create_task(_run_direct_shell_and_close())
            try:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from owls_cli.auth import has_usable_secret
