"""
Session-scoped context variables for the OWLS gateway.

Replaces the previous ``os.environ``-based session state
(``OWLS_SESSION_PLATFORM``, ``OWLS_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["OWLS_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("OWLS_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("OWLS_SESSION_PLATFORM", "")

    # after
    from gateway.session_context import get_session_env
    platform = get_session_env("OWLS_SESSION_PLATFORM", "")
"""
...
_SESSION_PLATFORM: ContextVar = ContextVar("OWLS_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("OWLS_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("OWLS_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("OWLS_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("OWLS_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("OWLS_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("OWLS_SESSION_KEY", default=_UNSET)

_VAR_MAP = {
    "OWLS_SESSION_PLATFORM": _SESSION_PLATFORM,
    "OWLS_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "OWLS_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "OWLS_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "OWLS_SESSION_USER_ID": _SESSION_USER_ID,
    "OWLS_SESSION_USER_NAME": _SESSION_USER_NAME,
    "OWLS_SESSION_KEY": _SESSION_KEY,
}
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``OWLS_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("OWLS_SESSION_*", default)``.
