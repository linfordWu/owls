"""
OWLS MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (OWLS-specific extra)

Usage:
    owls mcp serve
    owls mcp serve --verbose
...
logger = logging.getLogger("owls.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using OWLS_HOME."""
    try:
        from owls_constants import get_owls_home
        return get_owls_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("OWLS_HOME", Path.home() / ".owls")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from owls_constants import get_owls_home
        directory_file = get_owls_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("OWLS_HOME", Path.home() / ".owls")
...
class EventBridge:
    ...
    This is the OWLS equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from owls_constants import get_owls_home
            db_file = get_owls_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("OWLS_HOME", Path.home() / ".owls")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the OWLS MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "owls",
        instructions=(
            "OWLS messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the OWLS MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
