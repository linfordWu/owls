...
try:
    from owls_cli.plugins import discover_plugins
    discover_plugins()
except Exception as e:
...
def _apply_tool_preferences(filtered_tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Reorder tools based on user preferences from tool_preferences.json.

    Higher-priority tools are moved earlier in the list.  OpenAI models
    show a bias toward tools that appear earlier, so this gives users
    indirect control over tool selection priority.
    """
    import json as _json
    from pathlib import Path
    from owls_constants import get_owls_home

    prefs_path = get_owls_home() / "experience" / "tool_preferences.json"
    ...
    priorities = {name: info.get("priority", 0) for name, info in prefs.get("tools", {}).items()}
    if not priorities:
        return filtered_tools

    # Sort by priority descending, preserving original order for ties
    def _sort_key(item: Dict[str, Any]) -> tuple:
        name = item.get("function", {}).get("name", "")
        return (-priorities.get(name, 0), filtered_tools.index(item))

    return sorted(filtered_tools, key=_sort_key)
...
def get_tool_definitions(
    ...
    # Apply tool_preferences.json ordering (OpenAI prefers earlier tools)
    filtered_tools = _apply_tool_preferences(filtered_tools)
...
def handle_function_call(
    ...
    try:
        ...
        get_audit_logger().log_event({
            ...
            "tool_args": {"keys": list(function_args.keys())},
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from owls_cli.plugins import invoke_hook
                invoke_hook(
                    "pre_tool_call",
                    tool_name=function_name,
                    args=function_args,
        ...
        if _audit_logged:
            try:
                ...
                get_audit_logger().log_event({
                    ...
                    "task_id": task_id,
                    "tool_name": function_name,
                    "tool_result": {"summary": result_summary},
                    "description": f"Tool result: {function_name}",
                    "risk_level": "low",
                })
            except Exception:
                pass
