...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_owls_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:owls_pkce",
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `owls auth
    ...
    raise SystemExit(f"`owls auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # re-seeded on the next load_pool() call.
        ...
        print("Run `owls auth add openai-codex` to re-enable if needed.")
    ...
    elif removed.source == "owls_pkce" and provider == "anthropic":
        from owls_constants import get_owls_home
        oauth_file = get_owls_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared OWLS Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from owls_cli.auth import suppress_credential_source
        ...
        print("Run `owls auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `owls auth` is called bare."""
