"""owls claw — OpenClaw migration commands.

Usage:
    owls claw migrate              # Preview then migrate (always shows preview first)
    owls claw migrate --dry-run    # Preview only, no changes
    owls claw migrate --yes        # Skip confirmation prompt
    owls claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    owls claw cleanup              # Archive leftover OpenClaw directories
    owls claw cleanup --dry-run    # Preview what would be archived
"""
...
from owls_cli.config import get_owls_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "OWLS may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a OWLS gateway is running with connected platforms.
    ...
    print()
    print_error(
        "OWLS gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'owls stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_owls.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_owls", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: owls claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to OWLS")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'owls claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → OWLS migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: owls claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: owls claw migrate --yes")
            return
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: owls claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: owls claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  owls claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  owls claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  owls config set OPENROUTER_API_KEY sk-or-v1-...")
