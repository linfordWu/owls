"""Slash command definitions and autocomplete for the OWLS CLI.
...
from owls_cli.shell_completion import ShellCompletionEngine
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of OWLS config/state", "Session",
    ...
    CommandDef("agents", "Show active agents and running tasks", "Session",
               aliases=("tasks",)),
    CommandDef("team", "Manage AgentTeams roles and task dispatch", "Session",
               cli_only=True, args_hint="[roles|status|send|tail|stop]",
               subcommands=("roles", "status", "send", "tail", "stop")),
    CommandDef("queue", "Queue a prompt for the next turn (doesn't interrupt)", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("status", "Show session info", "Session"),
    CommandDef("shell", "Enter shell mode for direct Linux command execution", "Session",
               cli_only=True, args_hint="[exit|status]"),
    ...
    CommandDef("update", "Update OWLS to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    if not gated:
        return set()
    try:
        from owls_cli.config import read_raw_config
        cfg = read_raw_config()
    except Exception:
        return set()
    result: set[str] = set()
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``owls skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /owls handler.

    Maps both canonical names and aliases so /owls bg do stuff works
    the same as /owls background do stuff.
    """
    overrides = _resolve_config_gates()
...
class SlashCommandCompleter(Completer):
    ...
    def __init__(
        ...
        self._shell_completion_engine = ShellCompletionEngine()
    ...
    def _extract_current_word(text: str) -> tuple[str, str]:
        """Return ``(prefix_text, word)`` for the token under the cursor."""
        if not text:
            return "", ""
        i = len(text) - 1
        while i >= 0 and text[i] != " ":
    ...
    def _get_shell_command_names(self) -> list[str]:
        return self._shell_completion_engine._get_shell_command_names()

    def _get_git_subcommand_names(self) -> list[str]:
        return self._shell_completion_engine._get_git_subcommand_names()

    def _shell_command_completions(self, text: str, limit: int = 80):
        yield from self._shell_completion_engine.get_completions(text, limit)
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from owls_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def get_completions(self, document, complete_event):
        ...
        if not text.startswith("/"):
            ...
            if path_word is not None:
                ...
                return
            # Linux command completion is explicit-tab only so normal chat text
            # doesn't constantly surface shell suggestions while typing.
            if getattr(complete_event, "completion_requested", False):
                yield from self._shell_command_completions(text)
            return
        ...
        try:
            from owls_cli.plugins import get_plugin_commands
