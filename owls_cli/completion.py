"""Shell completion script generation for owls CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_owls_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# OWLS bash completion
# Add to ~/.bashrc:
#   eval "$(owls completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef owls
# OWLS zsh completion
# Add to ~/.zshrc:
#   eval "$(owls completion zsh)"
...
_owls() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_owls_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'owls command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# OWLS fish completion",
        "# Add to your config:",
        "#   owls completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __owls_profiles",
        "    echo default",
        "    if test -d $HOME/.owls/profiles",
        "        ls $HOME/.owls/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c owls -f -s p -l profile"
        " -d 'Profile name' -xa '(__owls_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__owls_profiles)' -d 'Profile name'"
                )
