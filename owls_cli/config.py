"""
Configuration management for OWLS.

Config files are stored in ~/.owls/ for easy access:
- ~/.owls/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.owls/.env         - API keys and secrets

This module provides:
- owls config          - Show current configuration
- owls config edit     - Open config in editor
- owls config set      - Set a specific value
- owls config wizard   - Re-run setup wizard
"""
...
from owls_cli.default_soul import DEFAULT_SOUL_MD, LEGACY_DEFAULT_SOUL_MARKERS
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("OWLS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_owls_home() / ".managed"
...
def is_managed() -> bool:
    """Check if OWLS is running in package-manager-managed mode.

    Two signals: the OWLS_MANAGED env var (set by the systemd service),
    or a .managed marker file in OWLS_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "owls update"


def format_managed_message(action: str = "modify this OWLS installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("OWLS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this OWLS installation is managed by NixOS "
            f"(OWLS_MANAGED={env_hint}).\n"
            "Edit services.owls.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this OWLS installation is managed by Homebrew "
            f"(OWLS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade owls"
        )
    ...
    return (
        f"Cannot {action}: this OWLS installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall OWLS."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from OWLS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, owls_bin
    or None if container mode is not active, we're already inside the
    container, or OWLS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("OWLS_DEV") == "1":
        return None
    ...
    container_mode_file = get_owls_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "owls")
    exec_user = info.get("exec_user", "owls")
    owls_bin = info.get("owls_bin", "/data/current-package/bin/owls")
...
# Re-export from owls_constants — canonical definition lives there.
from owls_constants import get_owls_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the owls group can
    share state with the gateway service.

    The mode can be overridden via the OWLS_HOME_MODE environment variable
    (e.g. OWLS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse OWLS_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("OWLS_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When OWLS runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("OWLS_CONTAINER") or os.environ.get("OWLS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set OWLS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed or refresh the stock SOUL.md in OWLS_HOME.

    User-customized SOUL.md files are left untouched.  Older unmodified stock
    templates are refreshed so CLI and Web Q&A share the current OWLS identity.
    """
    ...
    if soul_path.exists():
        try:
            ...
            return
        if current.strip() == DEFAULT_SOUL_MD:
            return
        if not all(marker in current for marker in LEGACY_DEFAULT_SOUL_MARKERS):
            return
...
def ensure_owls_home():
    """Ensure ~/.owls directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_owls_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_owls_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"OWLS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when OWLS
        ...
        #  "/home/user/.owls/cache/documents:/output"]
    ...
    "browser": {
        ...
        "camofox": {
            # When true, OWLS sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.owls/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.owls/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for owls-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *OWLS*" header.
    ...
    "code_execution": {
        ...
        #     owls's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.owls/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the OWLS auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @owls:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. OWLS_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'owls' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote OWLS API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote OWLS API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote OWLS API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # OWLS_TOOL_PROGRESS and OWLS_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.owls.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'owls setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — OWLS won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'owls doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("OWLS_HOME", "~/.owls")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("OWLS_TOOL_PROGRESS")
            old_mode = get_env_value("OWLS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from OWLS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from OWLS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("OWLS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from OWLS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: owls config set <key> <value>")
    ...
    # metadata.owls.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.owls/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.owls/config.yaml."""
    ensure_owls_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — owls auth) — OpenAI Codex
#   nous         (OAuth — owls auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.owls/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.owls/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real OWLS env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.owls/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.owls/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.owls/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.owls/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to OWLS — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known OWLS vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.owls/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ OWLS Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  owls config edit     # Edit config file", Colors.DIM))
    print(color("  owls config set <key> <value>", Colors.DIM))
    print(color("  owls setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: owls config set <key> <value>")
            print()
            print("Examples:")
            print("  owls config set model anthropic/claude-sonnet-4")
            print("  owls config set terminal.backend docker")
            print("  owls config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'owls config migrate' to add them")
        ...
        print("  owls config           Show current configuration")
        print("  owls config edit      Open config in editor")
        print("  owls config set <key> <value>   Set a config value")
        print("  owls config check     Check for missing/outdated config")
        print("  owls config migrate   Update config with new options")
        print("  owls config path      Show config file path")
        print("  owls config env-path  Show .env file path")
        sys.exit(1)
