"""``owls debug`` — debug tools for OWLS.

Currently supports:
    owls debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    """Path to ``~/.owls/pastes/pending.json``.
    ...
    ran ``owls debug share`` repeatedly.  We now persist the schedule
    to disk and sweep expired entries on the next debug invocation.
    """
    return get_owls_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``owls debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``owls debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, OWLS version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `owls debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "owls/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``owls debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.owls/pastes/pending.json``
    and rely on opportunistic sweeps (``_sweep_expired_pastes``) called from
    every ``owls debug`` invocation.  If the user never runs ``owls debug``
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"owls debug delete {url}"
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----OWLSDebugBoundary9f3c"
...
def _resolve_log_path(log_name: str) -> Optional[Path]:
    ...
    """
    from owls_cli.logs import LOG_FILES
...
def _read_log_tail(log_name: str, num_lines: int) -> str:
    ...
    from owls_cli.logs import _read_last_n_lines
...
def _capture_dump() -> str:
    """Run ``owls dump`` and return its stdout as a string."""
...
def collect_debug_report(*, log_lines: int = 200, dump_text: str = "") -> str:
    ...
    dump_text
        Pre-captured dump output.  If empty, ``owls dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    print(f"To delete now:  owls debug delete <url>")

    print(f"\nShare these links with the OWLS team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: owls debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'owls debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``owls debug`` call.
    ...
    # best-effort — any failure is swallowed so ``owls debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: owls debug <command>")
        print()
        print("Commands:")
