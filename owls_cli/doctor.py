"""
Doctor command for owls CLI.

Diagnoses issues with OWLS setup.
"""
...
from owls_cli.config import get_project_root, get_owls_home, get_env_path
from owls_constants import display_owls_home

PROJECT_ROOT = get_project_root()
OWLS_HOME = get_owls_home()
_DHH = display_owls_home()  # user-facing display path (e.g. ~/.owls or ~/.owls/profiles/coder)

# Load environment variables from ~/.owls/.env so API key checks work
...
from owls_constants import OPENROUTER_MODELS_URL
...
from owls_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.owls/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `owls`.
    os.environ.setdefault("OWLS_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 OWLS Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.owls/.env (primary location for user config)
    env_path = OWLS_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'owls setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'owls setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'owls setup' to create one")
                issues.append("Run 'owls setup' to create .env")
    
    # Check ~/.owls/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = OWLS_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from owls_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'owls setup' to migrate config")
                else:
                    issues.append("Run 'owls doctor --fix' or 'owls setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'owls doctor --fix'")
        except Exception:
            pass
    ...
    if owls_home.exists():
        ...
        if should_fix:
            owls_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = owls_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give OWLS a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# OWLS Persona\n\n"
                "<!-- Edit this file to customize how OWLS communicates. -->\n\n"
    ...
    memories_dir = owls_home / "memories"
    ...
    state_db_path = owls_home / "state.db"
    ...
    wal_path = owls_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'owls doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (owls bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "owls"
        ...
        _cmd_link = _cmd_link_dir / "owls"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(owls not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/owls → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/owls points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/owls → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/owls — run 'owls doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/owls exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/owls not found",
                    "(owls command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/owls → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/owls symlink — run 'owls doctor --fix'")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'owls setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: owls skills list)")
    ...
    try:
        ...
        _mem_cfg_path = OWLS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: owls memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: owls memory setup")
                issues.append("No Honcho API key — run 'owls memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run owls memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: owls memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: owls memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from owls_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "owls -p" in content:
                            _m = _re.search(r"owls -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'owls doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
