"""
Dump command for owls CLI.

Outputs a compact, plain-text summary of the user's OWLS setup
...
from owls_cli.config import get_owls_home, get_env_path, get_project_root, load_config
...
def _gateway_status() -> str:
    ...
    try:
        from owls_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(owls_home: Path) -> int:
...
def _cron_summary(owls_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = owls_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"owls_home:      {display_owls_home()}")
    ...
    toolsets = config.get("toolsets", ["owls-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(owls_home)}")
    lines.append(f"  skills:             {_count_skills(owls_home)}")
