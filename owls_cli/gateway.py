"""
Gateway subcommand for owls CLI.

Handles: owls gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_owls_home is imported lazily at call sites to avoid ImportError
# when owls_constants is cached from a pre-update version during `owls update`.
from owls_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from owls_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "owls-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "owls_cli.main gateway",
        "owls_cli.main --profile",
        "owls_cli.main -p",
        "owls_cli/main.py gateway",
        "owls_cli/main.py --profile",
        "owls_cli/main.py -p",
        "owls gateway",
        "gateway/run.py",
    ]
    current_home = str(get_owls_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"OWLS_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "OWLS_HOME=" in command and f"OWLS_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``owls update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            OWLS profile are returned.
    """
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from owls_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `owls gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (OWLS_HOME-scoped).
...
from owls_constants import is_container, is_termux
...
_SERVICE_BASE = "owls-gateway"
SERVICE_DESCRIPTION = "OWLS Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current OWLS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (OWLS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from owls_constants import get_default_owls_root
    home = get_owls_home().resolve()
    default = get_default_owls_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary OWLS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(owls_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when OWLS_HOME is a named profile.

    For ``~/.owls/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        owls_home: Optional explicit OWLS_HOME path. Defaults to the current
            ``get_owls_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from owls_constants import get_default_owls_root
    home = Path(owls_home or str(get_owls_home())).resolve()
    default = get_default_owls_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this OWLS_HOME.

    Default ``~/.owls`` returns ``owls-gateway`` (backward compatible).
    Profile ``~/.owls/profiles/coder`` returns ``owls-gateway-coder``.
    Any other OWLS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older OWLS installs that predate the
# owls-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (owls-gateway-*.service) and unrelated third-party
# "owls" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("owls.service",)
...
def _find_legacy_owls_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy OWLS gateway units.

    Detects unit files installed by older OWLS versions that used a
    different service name (e.g. ``owls.service`` before the rename to
    ``owls-gateway.service``). When both a legacy unit and the current
    ``owls-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``owls-gateway-coder.service`` and unrelated third-party
      ``owls-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``owls.service`` running an unrelated
...
def has_legacy_owls_units() -> bool:
    """Return True when any legacy OWLS gateway unit files exist."""
    return bool(_find_legacy_owls_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy OWLS gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_owls_units()
    if not legacy:
        return
    print_warning("Legacy OWLS gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current owls-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    owls gateway migrate-legacy")


def remove_legacy_owls_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy OWLS gateway unit files.

    Iterates over whatever ``_find_legacy_owls_units()`` returns — which is
    ...
    legacy = _find_legacy_owls_units()
    if not legacy:
        print("No legacy OWLS gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy OWLS gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: owls gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo owls gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    owls gateway uninstall")
    print_info("    sudo owls gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so OWLS can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo owls gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo owls gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo owls gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode OWLS often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.owls`` → ``ai.owls.gateway.plist`` (backward compatible).
    Profile ``~/.owls/profiles/coder`` → ``ai.owls.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.owls.gateway-{suffix}" if suffix else "ai.owls.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.owls/owls  -> /home/alice/.owls/owls
      /opt/owls                 -> /opt/owls  (kept as-is)
...
def _owls_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current OWLS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_owls_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.owls                    → /home/alice/.owls
      /root/.owls/profiles/coder     → /home/alice/.owls/profiles/coder
      /opt/custom-owls               → /opt/custom-owls  (kept as-is)
    """
    current_owls = get_owls_home().resolve()
    current_default = (Path.home() / ".owls").resolve()
    target_default = Path(target_home_dir) / ".owls"

    # Default ~/.owls → remap to target user's default
    if current_owls == current_default:
        return str(target_default)

    # Profile or subdir of ~/.owls → preserve the relative structure
    try:
        relative = current_owls.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.owls) — keep as-is
        return str(current_owls)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        owls_home = _owls_home_for_target_user(home_dir)
        profile_arg = _profile_arg(owls_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m owls_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="OWLS_HOME={owls_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    owls_home = str(get_owls_home().resolve())
    profile_arg = _profile_arg(owls_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current OWLS install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (owls.service from pre-rename installs)
    # before installing the new owls-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}owls gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}owls gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}owls gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}owls gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}owls gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}owls gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.owls.gateway-{suffix}" if suffix else "ai.owls.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current OWLS install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  owls gateway status             # Check status")
    from owls_constants import display_owls_home as _dhh
...
def launchd_stop():
    ...
    # `owls gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current OWLS install")
    else:
        print("⚠ Service definition is stale relative to the current OWLS install")
        print("  Run: owls gateway start")
    ...
    if deep:
        log_file = get_owls_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and OWLS will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @owls:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. owls) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where OWLS delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your OWLS agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address OWLS will use (e.g., owls@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. OWLS connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: owls pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_owls_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'owls pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: owls pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'owls gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from owls_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from owls_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. OWLS will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. OWLS will store the returned account_id/token in ~/.owls/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `owls gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_owls_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `owls pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `owls pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"OWLS\"")
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: owls gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: owls gateway install")
                else:
                    print_info("  You can install later: owls gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo owls gateway install --system")
                    print_info("  Or run in foreground:  owls gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: owls gateway run")
                print_info("  For persistence:   tmux new -s owls 'owls gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: owls gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup owls gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: owls gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'owls gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: owls gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: owls gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s owls 'owls gateway run'")
                print()
            ...
            print()
            print("  owls gateway run                              # direct foreground")
            print("  tmux new -s owls 'owls gateway run'         # persistent via tmux")
            print("  nohup owls gateway run > ~/.owls/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: owls gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: owls gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: owls gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: owls gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: owls gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  owls gateway install")
                    print("  sudo owls gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  owls gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup owls gateway run > ~/.owls/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s owls 'owls gateway run'         # persistent via tmux")
                    print("  nohup owls gateway run > ~/.owls/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  owls gateway install  # Install as user service")
                    print("  sudo owls gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy OWLS gateway unit files from
        # pre-rename installs (e.g. owls.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_owls_units(interactive=not yes, dry_run=dry_run)
