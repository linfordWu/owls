"""``owls logs`` — view and filter OWLS log files.
...
Usage examples::
    ...
    owls logs                    # last 50 lines of agent.log
    owls logs -f                 # follow agent.log in real time
    owls logs errors             # last 50 lines of errors.log
    owls logs gateway -n 100    # last 100 lines of gateway.log
    owls logs --level WARNING    # only WARNING+ lines
    owls logs --session abc123   # filter by session ID substring
    owls logs --component tools  # only tool-related lines
    owls logs --since 1h         # lines from the last hour
    owls logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_owls_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when OWLS runs — try 'owls chat' first)")
        sys.exit(1)
    ...
    if component:
        from owls_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_owls_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_owls_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_owls_home()}/logs/")
        return

    print(f"Log files in {display_owls_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'owls chat' to generate logs)")
