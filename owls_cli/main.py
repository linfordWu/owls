#!/usr/bin/env python3
"""
OWLS CLI - Main entry point.

Usage:
    owls                     # Interactive chat (default)
    owls chat                # Interactive chat
    owls gateway             # Run web/API backend in foreground
    owls gateway start       # Start gateway as service
    owls gateway stop        # Stop gateway service
    owls gateway status      # Show gateway status
    owls gateway install     # Install gateway service
    owls gateway uninstall   # Uninstall gateway service
    owls setup               # Interactive setup wizard
    owls logout              # Clear stored authentication
    owls status              # Show status of all components
    owls cron                # Manage cron jobs
    owls cron list           # List cron jobs
    owls cron status         # Check if cron scheduler is running
    owls doctor              # Check configuration and dependencies
    owls honcho setup                    # Configure Honcho AI memory integration
    owls honcho status                   # Show Honcho config and connection status
    owls honcho sessions                 # List directory → session name mappings
    owls honcho map <name>               # Map current directory to a session name
    owls honcho peer                     # Show peer names and dialectic settings
    owls honcho peer --user NAME         # Set user peer name
    owls honcho peer --ai NAME           # Set AI peer name
    owls honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    owls honcho mode                     # Show current memory mode
    owls honcho mode [hybrid|honcho|local]  # Set memory mode
    owls honcho tokens                   # Show token budget settings
    owls honcho tokens --context N       # Set session.context() token cap
    owls honcho tokens --dialectic N     # Set dialectic result char cap
    owls honcho identity                 # Show AI peer identity representation
    owls honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    owls honcho migrate                  # Step-by-step migration guide: OpenClaw native → OWLS + Honcho
    owls version             Show version
    owls update              Update to latest version
    owls uninstall           Uninstall OWLS
    owls sessions browse     Interactive session picker with search

    owls claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (owls tools, owls setup, owls model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'owls {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any owls module import.
#
# Many modules cache OWLS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("OWLS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.owls/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set OWLS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the owls root
    if profile_name is None:
        try:
            from owls_constants import get_default_owls_root

            active_path = get_default_owls_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set OWLS_HOME
    if profile_name is not None:
        try:
            from owls_cli.profiles import resolve_profile_env

            owls_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent owls from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["OWLS_HOME"] = owls_home
...
from owls_cli.config import get_owls_home
from owls_cli.env_loader import load_owls_dotenv

load_owls_dotenv(project_env=PROJECT_ROOT / ".env")

# Initialize centralized file logging early — all `owls` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from owls_logging import setup_logging as _setup_logging
...
try:
    from owls_cli.config import load_config as _load_config_early
    from owls_constants import apply_ipv4_preference as _apply_ipv4
...
from owls_cli import __version__, __release_date__
from owls_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from owls_cli.config import get_env_path, get_owls_home, load_config
    from owls_cli.auth import get_auth_status

    # Determine whether OWLS itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from owls_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_owls_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from owls_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_owls_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if OWLS has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants OWLS to use their tokens.
    if _has_owls_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, owls_bin
        cli_args: the original CLI arguments (everything after 'owls')
    """
    import shutil
    ...
    owls_bin = container_info["owls_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo owls {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo owls {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str]) -> None:
    ...
    print(f"  owls --tui --resume {target}")
    if title:
        print(f'  owls --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @owls/ink is missing or node_modules is behind package-lock.json (post-pull)."""
    ink = root / "node_modules" / "@owls" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: OWLS_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("OWLS_TUI_DIR")
    if env:
        p = Path(env)
...
def _owls_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "owls-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``OWLS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("OWLS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    owls_home = os.environ.get("OWLS_HOME") or str(Path.home() / ".owls")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "OWLS_HOME": owls_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(owls_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (OWLS_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("OWLS_NODE")
    ...
    # pre-built dist + node_modules (nix / full OWLS_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("OWLS_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("OWLS_QUIET"):
    ...
    if tui_dev:
        if _owls_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/owls-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _launch_tui(resume_session_id: Optional[str] = None, tui_dev: bool = False):
    ...
    env = os.environ.copy()
    env["OWLS_PYTHON_SRC_ROOT"] = os.environ.get(
        "OWLS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("OWLS_PYTHON", sys.executable)
    env.setdefault("OWLS_CWD", os.getcwd())
    if resume_session_id:
        env["OWLS_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("OWLS_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'owls sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like OWLS isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  owls setup")
        print()

        from owls_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'owls setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from owls_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["OWLS_SESSION_SOURCE"] = args.source
    ...
    kwargs = {
        ...
        "team_worker": getattr(args, "team_worker", False),
        "team_name": getattr(args, "team_name", None),
        "team_role": getattr(args, "team_role", None),
    }
...
def cmd_gateway(args):
    ...
    from owls_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from owls_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with OWLS?")
        print()
    ...
    session_dir = get_owls_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: owls gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  owls gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ OWLS'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  owls gateway")
        ...
        print()
        print("  Or install as a service: owls gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'owls whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from owls_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``owls model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from owls_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    effective_provider = (
        config_provider or os.getenv("OWLS_INFERENCE_PROVIDER") or "auto"
    )
    try:
    ...
    from owls_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.owls/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.owls/.env if the active provider is not 'custom'.
    ...
    """
    from owls_cli.config import get_env_value, save_env_value, load_config
...
#
# OWLS uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `owls model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `owls model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from owls_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  OWLS only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``owls model`` flow, then route aux tasks to them here.
    """
    from owls_cli.config import load_config
    from owls_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from owls_cli.auth import _prompt_model_selection
    from owls_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from owls_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from owls_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from owls_cli.nous_subscription import prompt_enable_tool_gateway
    import argparse
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from owls_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `owls auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from owls_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from owls_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    from owls_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from owls_cli.auth import _save_model_choice, deactivate_provider
    from owls_cli.config import get_env_value, load_config, save_config
    ...
    from owls_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `owls model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from owls_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from owls_cli.config import save_env_value, load_config, save_config
    from owls_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from owls_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from owls_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates OWLS turns to `copilot --acp`.")
    print("  OWLS currently starts its own ACP subprocess for each request.")
    print("  OWLS uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set OWLS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so owls knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "ollama-cloud":
        from owls_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from owls_constants import display_owls_home as _dhh_fn

            print(
                f"    OWLS will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from owls_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'owls model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate OWLS CLI with a provider."""
    from owls_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from owls_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from owls_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from owls_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from owls_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from owls_cli.webhook import webhook_command
...
def cmd_doctor(args):
    ...
    from owls_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from owls_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from owls_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from owls_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up OWLS home directory to a zip file."""
    if getattr(args, "quick", False):
        from owls_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from owls_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a OWLS backup from a zip file."""
    from owls_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"OWLS v{__version__} ({__release_date__})")
    ...
    try:
        from owls_cli.banner import check_for_updates
        from owls_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from owls_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``owls update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``owls web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (owls web will not be available)")
        )
        if fatal:
...
def _update_via_zip(args):
    """Update OWLS by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/owls/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="owls-update-")
        zip_path = os.path.join(tmp_dir, f"owls-{branch}.zip")
        ...
        # GitHub ZIPs extract to owls-<branch>/
        extracted = os.path.join(tmp_dir, f"owls-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "owls-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if OWLS behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes owls completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but OWLS couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but OWLS couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if OWLS behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `owls update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/owls.git",
    "git@github.com:NousResearch/owls.git",
    "https://github.com/NousResearch/owls",
    "git@github.com:NousResearch/owls",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/owls.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_owls_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_owls_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official OWLS repository.")
        print("  This means you may miss updates from NousResearch/owls.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/owls.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/owls.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``owls update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from owls_constants import get_default_owls_root

    default_home = get_default_owls_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "owls[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``owls update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.owls/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``owls update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``owls update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.owls/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``owls update --gateway``) the update is already
...
def cmd_update(args):
    """Update OWLS to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from owls_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update OWLS")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/owls/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_owls_home added to owls_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload owls_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_owls_home() added since the last release.
        try:
            import importlib
            import owls_constants as _hc
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'owls config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: owls config migrate")
            else:
                print()
                print("Skipped. Run 'owls config migrate' later to configure.")
        else:
        ...
        # When running as ``owls update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart owls-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_owls_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all owls-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. owls-gateway.service or owls-gateway-coder.service
            ...
            # immediately kill the process that systemd just spawned.
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: owls gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: owls -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy OWLS gateway unit files are still installed.
        # When both owls.service (from a pre-rename install) and the
        # current owls-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `owls update` surfaces the issue until the user migrates.
        try:
            from owls_cli.gateway import (
                has_legacy_owls_units,
                _find_legacy_owls_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_owls_units():
                print()
                print("⚠ Legacy OWLS gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_owls_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (owls.service) fight the current")
                print("  owls-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    owls gateway migrate-legacy")
                print()
        ...
        print("  owls model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``owls -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `owls profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_owls_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → owls -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.owls)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  owls profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     owls -p {name} chat")
                else:
        ...
        from owls_cli.profiles import profile_exists
        ...
        from owls_cli.profiles import rename_profile
        ...
        from owls_cli.profiles import export_profile
        ...
        from owls_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    from owls_cli.web_ui_launcher import start_web_ui
...
def cmd_completion(args, parser=None):
    ...
    from owls_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter OWLS log files."""
    from owls_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for owls CLI."""
    parser = argparse.ArgumentParser(
        prog="owls",
        description="OWLS - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    owls                        Start interactive chat
    owls chat -q "Hello"        Single query mode
    owls -c                     Resume the most recent session
    owls -c "my project"        Resume a session by name (latest in lineage)
    owls --resume <session_id>  Resume a specific session by ID
    owls setup                  Run setup wizard
    owls logout                 Clear stored authentication
    owls auth add <provider>    Add a pooled credential
    owls auth list              List pooled credentials
    owls auth remove <p> <t>    Remove pooled credential by index, id, or label
    owls auth reset <provider>  Clear exhaustion status for a provider
    owls model                  Select default model
    owls config                 View configuration
    owls config edit            Edit config in $EDITOR
    owls config set model gpt-4 Set a config value
    owls gateway                Run messaging gateway
    owls -s owls-dev,github-auth
    owls -w                     Start in isolated git worktree
    owls gateway install        Install gateway background service
    owls sessions list          List past sessions
    owls sessions browse        Interactive session picker
    owls sessions rename ID T   Rename/title a session
    owls logs                   View agent.log (last 50 lines)
    owls logs -f                Follow agent.log in real time
    owls logs errors            View errors.log
    owls logs --since 1h        Lines from the last hour
    owls debug share             Upload debug report for support
    owls update                 Update to latest version
...
""",
    ...
    parser.add_argument(
        ...
        choices=["true", "tmux"],
        default=None,
        help="Arm AgentTeams: 'true' for background workers, 'tmux' for split-screen workers after prompt-defined roles",
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with OWLS",
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: owls-cli)",
    )
    model_parser.add_argument(
    ...
    gateway_parser = subparsers.add_parser(
        "gateway",
        help="Linux web/API backend management",
        description="Manage the Linux OWLS web/API backend service",
    )
    gateway_subparsers = gateway_parser.add_subparsers(dest="gateway_command")

    # gateway run (default)
    gateway_run = gateway_subparsers.add_parser(
        "run", help="Run the web/API backend in foreground"
    )
    ...
    gateway_start = gateway_subparsers.add_parser(
        "start", help="Start the installed systemd background service"
    )
    ...
    gateway_install = gateway_subparsers.add_parser(
        "install", help="Install gateway as a systemd background service"
    )
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy owls.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy OWLS gateway unit files "
            "(e.g. owls.service) left over from older installs. Profile "
            "units (owls-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure OWLS with an interactive wizard. "
        "Run a specific section: owls setup model|tts|terminal|tools|agent",
    )
    setup_parser.add_argument(
        "section",
        nargs="?",
        choices=["model", "tts", "terminal", "tools", "agent"],
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for OWLS CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: owls-cli)"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of OWLS components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with OWLS setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your OWLS setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for OWLS. Use 'owls debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    owls debug share              Upload debug report and print URL
    owls debug share --lines 500  Include more log lines
    owls debug share --expire 30  Keep paste for 30 days
    owls debug share --local      Print report locally (no upload)
    owls debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'owls debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up OWLS home directory to a zip file",
        description="Create a zip archive of your entire OWLS configuration, "
        "skills, sessions, and data (excludes the owls codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/owls-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a OWLS backup from a zip file",
        description="Extract a previously created OWLS backup into your "
        "OWLS home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage OWLS configuration",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.owls/skills/.bundled_manifest) "
            "so future 'owls update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from owls_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from owls_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/owls-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    def cmd_plugins(args):
        from owls_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_owls_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_owls_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_owls_home()}/memories/\n")
        else:
            from owls_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'owls tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # owls tools disable <name...> [--platform cli]
    ...
    # owls tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            _require_tty("tools")
            from owls_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run OWLS as an MCP server",
        description=(
            "Manage MCP server connections and run OWLS as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'owls mcp add' to connect to a new server, or\n"
            "'owls mcp serve' to expose OWLS conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run OWLS as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from owls_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch owls --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil

            owls_bin = shutil.which("owls")
            if owls_bin:
                os.execvp(owls_bin, ["owls", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "owls_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to OWLS",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to OWLS",
    ...
    def cmd_claw(args):
        from owls_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update OWLS to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall OWLS",
        description="Remove OWLS from your system. Can keep configs/data for reinstall.",
    )
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated OWLS instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the OWLS web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter OWLS log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    owls logs                    Show last 50 lines of agent.log
    owls logs -f                 Follow agent.log in real time
    owls logs errors             Show last 50 lines of errors.log
    owls logs gateway -n 100     Show last 100 lines of gateway.log
    owls logs --level WARNING    Only show WARNING and above
    owls logs --session abc123   Filter by session ID
    owls logs --component tools  Only show tool-related lines
    owls logs --since 1h         Lines from the last hour
    owls logs --since 30m -f     Follow, starting from 30 min ago
    owls logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``owls -c Pokemon Agent Dev`` → ``owls -c 'Pokemon Agent Dev'``
    ...
    from owls_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'owls -c model' where 'model' is consumed as the
