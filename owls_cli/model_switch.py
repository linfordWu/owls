...
- ``owls_cli.providers``        -- canonical provider identity + overlays
- ``owls_cli.model_normalize``  -- per-provider name formatting
...
from owls_cli.providers import (
...
_OWLS_MODEL_WARNING = (
    "Nous Research OWLS 3 & 4 models are NOT agentic and are not designed "
    "for use with OWLS. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research OWLS 3 / OWLS 4 chat families.
# The previous substring check (`"owls" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``owls-brain:qwen3-14b-ctx16k`` that just
# happen to carry "owls" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/OWLS-3-Llama-3.1-70B, owls-4-405b, openrouter/ospilot3:70b
# Negative examples it must NOT match:
#   owls-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_OWLS_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])owls[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_owls_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous OWLS 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_OWLS_NON_AGENTIC_RE.search(model_name))


def _check_owls_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous OWLS 3/4 chat model."""
    if is_nous_owls_non_agentic(model_name):
        return _OWLS_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    from owls_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'owls model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'owls doctor' — config issues detected:"
    ...
    # Mirrors the same logic in owls_cli.runtime_provider.resolve_runtime_provider;
    ...
    owls_warn = _check_owls_model_warning(new_model)
    if owls_warn:
        warnings.append(owls_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from owls_cli/models.py (OPENROUTER_MODELS,
    ...
    from owls_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by owls provider ID
    ...
    # --- 1. Check OWLS-mapped providers ---
    for owls_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(owls_id)
    ...
    # --- 2. Check OWLS-only providers (nous, openai-codex, copilot, opencode-go) ---
    from owls_cli.providers import OWLS_OVERLAYS
    from owls_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → OWLS provider ID.
    # OWLS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use OWLS IDs ("copilot").
    _mdev_to_owls = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in OWLS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve OWLS slug — e.g. "github-copilot" → "copilot"
        owls_slug = _mdev_to_owls.get(pid, pid)
        if owls_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from owls_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or owls_slug in providers_store
                    or pid in pool_store or owls_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(owls_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", owls_slug, exc)
        ...
        # configured.
        if not has_creds and owls_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_owls_oauth_credentials,
                )
                owls_creds = read_owls_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (owls_creds and owls_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by OWLS slug, fall back to overlay key
        model_ids = curated.get(owls_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": owls_slug,
            "name": get_label(owls_slug),
            "is_current": owls_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "owls",
        ...
        seen_slugs.add(owls_slug.lower())

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or OWLS_OVERLAYS (keeps /model in sync
    # with `owls model`).
    try:
        from owls_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is OWLS's canonical write key (matches
            ...
            # OWLS writes ``models:`` as a dict keyed by model id
            # (see owls_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. OWLS's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, owls_cli/config.py) already consume that dict.
