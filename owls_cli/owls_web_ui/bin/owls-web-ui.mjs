...
const PID_DIR = resolve(homedir(), '.owls-web-ui')
...
function startDaemon(port) {
  ...
  if (existing && isRunning(existing)) {
    console.log(`  ✗ owls-web-ui is already running (PID: ${existing})`)
    console.log(`    Use "owls-web-ui stop" to stop it first`)
    process.exit(1)
  }
  removePid()
  ...
  console.log(`  ⏳ Starting owls-web-ui (PID: ${child.pid}, port: ${port})...`)

  function poll() {
    waited += interval
    if (!isRunning(child.pid)) {
      console.log('  ✗ Failed to start owls-web-ui')
    ...
    fetch(healthUrl).then(res => {
      if (res.ok) {
        const url = token
          ...
          : `http://localhost:${port}`
        console.log(`  ✓ owls-web-ui started`)
        console.log(`    ${url}`)
...
function stopDaemon() {
  const pid = getPid()
  if (!pid) {
    console.log('  ✗ owls-web-ui is not running')
    process.exit(1)
  }

  if (!isRunning(pid)) {
    removePid()
    console.log(`  ✓ owls-web-ui was not running (cleaned stale PID)`)
    return
  }
  ...
  try {
    ...
    removePid()
    console.log(`  ✓ owls-web-ui stopped (PID: ${pid})`)
  } catch (err) {
...
function showStatus() {
  ...
  if (pid && isRunning(pid)) {
    console.log(`  ✓ owls-web-ui is running (PID: ${pid})`)
...
if (['-v', '--version', 'version'].includes(command)) {
  console.log(`owls-web-ui v${VERSION}`)
  process.exit(0)
}

if (['-h', '--help', 'help'].includes(command)) {
  console.log(`
owls-web-ui v${VERSION}

Usage: owls-web-ui <command> [options]
...
function doUpdate() {
  console.log('  ⬆ Updating owls-web-ui...')

  const child = spawnCli(getNpmBin(), ['install', '-g', 'owls-web-ui@latest'], {
    stdio: 'inherit',
    windowsHide: true,
  })
