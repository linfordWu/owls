...
export async function startRun(body: StartRunRequest): Promise<StartRunResponse> {
  return request<StartRunResponse>('/api/owls/v1/runs', {
    method: 'POST',
...
export function streamRunEvents(
  ...
  const profile = localStorage.getItem('owls_active_profile_name')
  const params = new URLSearchParams()
  if (token) params.set('token', token)
  if (profile && profile !== 'default') params.set('profile', profile)
  const qs = params.toString()
  const url = `${baseUrl}/api/owls/v1/runs/${runId}/events${qs ? `?${qs}` : ''}`
...
export async function fetchModels(): Promise<{ data: Array<{ id: string }> }> {
  return request('/api/owls/v1/models')
}
