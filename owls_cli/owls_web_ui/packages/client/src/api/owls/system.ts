...
export async function triggerUpdate(): Promise<{ success: boolean; message: string }> {
  return request<{ success: boolean; message: string }>('/api/owls/update', { method: 'POST' })
}

export async function fetchConfigModels(): Promise<ConfigModelsResponse> {
  return request<ConfigModelsResponse>('/api/owls/config/models')
}

export async function fetchAvailableModels(): Promise<AvailableModelsResponse> {
  return request<AvailableModelsResponse>('/api/owls/available-models')
}
...
export async function updateDefaultModel(data: {
  ...
  await request('/api/owls/config/model', {
    method: 'PUT',
...
export async function addCustomProvider(data: CustomProvider): Promise<void> {
  await request('/api/owls/config/providers', {
    method: 'POST',
    body: JSON.stringify(data),
  })
}
...
export async function removeCustomProvider(name: string): Promise<void> {
  await request(`/api/owls/config/providers/${encodeURIComponent(name)}`, {
    method: 'DELETE',
  })
}
...
export async function updateProvider(poolKey: string, data: {
  ...
  await request(`/api/owls/config/providers/${encodeURIComponent(poolKey)}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  })
}
