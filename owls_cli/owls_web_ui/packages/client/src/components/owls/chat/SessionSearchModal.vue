...
import { fetchSessions, searchSessions, type SessionSearchResult, type SessionSummary } from '@/api/owls/sessions'
...
async function openItem(item: SearchItem) {
  ...
  if (router.currentRoute.value.name !== 'owls.chat') {
    await router.push({ name: 'owls.chat' })
  }
}
