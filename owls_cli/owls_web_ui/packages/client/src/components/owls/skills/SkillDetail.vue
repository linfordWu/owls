...
import MarkdownRenderer from '@/components/owls/chat/MarkdownRenderer.vue'
import { fetchSkillContent, fetchSkillFiles, type SkillFileEntry } from '@/api/owls/skills'
...
async function loadSkill() {
  ...
  try {
    const [skillContent, skillFiles] = await Promise.all([
      fetchSkillContent(`${props.skillPath}/SKILL.md`),
      fetchSkillFiles(props.skillPath),
    ])
...
async function viewFile(filePath: string) {
  ...
  try {
    // filePath might be absolute or relative; normalize to relative under the skill path.
    const base = `${props.skillPath}/`
    let relPath = filePath
    if (filePath.startsWith('/')) {
      // Strip absolute prefix to get relative path
      const segments = filePath.split('/.owls/skills/')[1]
      if (segments) {
        const skillParts = props.skillPath.split('/').length
        const afterSkillDir = segments.split('/').slice(skillParts).join('/')
...
watch(() => props.skillPath, loadSkill, { immediate: true })
</script>
