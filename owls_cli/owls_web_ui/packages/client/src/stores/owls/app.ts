...
import { ref } from 'vue'
import { checkHealth, fetchAvailableModels, updateDefaultModel, triggerUpdate, type AvailableModelGroup } from '@/api/owls/system'
import { fetchAuthStatus } from '@/api/auth'

const WEB_UI_VERSION = __APP_VERSION__

const SIDEBAR_COLLAPSED_KEY = 'owls_sidebar_collapsed'
...
export const useAppStore = defineStore('app', () => {
  ...
  const currentUserRole = ref<'admin' | 'user' | null>(null)
  ...
  async function loadAuthStatus() {
    ...
    try {
      ...
      currentUserRole.value = status.role
    } catch {
