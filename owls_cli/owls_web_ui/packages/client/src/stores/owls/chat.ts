import { startRun, streamRunEvents, type ChatMessage, type RunEvent } from '@/api/owls/chat'
import { deleteSession as deleteSessionApi, fetchSession, fetchSessions, fetchSessionUsageSingle, type OwlsMessage, type SessionSummary } from '@/api/owls/sessions'
...
function mapOwlsMessages(msgs: OwlsMessage[]): Message[] {
...
function mapOwlsSession(s: SessionSummary): Session {
  return {
    id: s.id,
    title: s.title || '',
...
const STORAGE_KEY_PREFIX = 'owls_active_session_'
const SESSIONS_CACHE_KEY_PREFIX = 'owls_sessions_cache_v1_'
const LEGACY_STORAGE_KEY = 'owls_active_session'
...
function getUserScope(): string {
  return (localStorage.getItem('owls_api_key') || 'anonymous').slice(0, 16)
}

function cacheScope(): string {
  return `${getProfileName()}_${getUserScope()}`
}

function storageKey(): string { return STORAGE_KEY_PREFIX + cacheScope() }
function sessionsCacheKey(): string { return SESSIONS_CACHE_KEY_PREFIX + cacheScope() }
function msgsCacheKey(sid: string): string { return `owls_session_msgs_v1_${cacheScope()}_${sid}_` }
function inFlightKey(sid: string): string { return `owls_in_flight_v1_${cacheScope()}_${sid}` }
function legacyStorageKey(): string | null { return null }
function legacySessionsCacheKey(): string | null { return null }
function legacyMsgsCacheKey(_sid: string): string | null { return null }
function legacyInFlightKey(_sid: string): string | null { return null }
...
function recoverStorageQuota() {
  try {
    const prefixes = [
      sessionsCacheKey(),
      `owls_session_msgs_v1_${getProfileName()}_`,
      `owls_in_flight_v1_${getProfileName()}_`,
    ]
    const legacySessions = legacySessionsCacheKey()
    if (legacySessions) prefixes.push(legacySessions)
    if (getProfileName() === 'default') {
      prefixes.push('owls_session_msgs_v1_')
      prefixes.push('owls_in_flight_v1_')
    }
...
export const useChatStore = defineStore('chat', () => {
  ...
  function startPolling(sid: string) {
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (!detail) return
        const mapped = mapOwlsMessages(detail.messages || [])
  ...
  async function loadSessions() {
    ...
    try {
      ...
      const fresh = list.map(mapOwlsSession)
  ...
  async function switchSession(sessionId: string, focusId?: string | null) {
    ...
    try {
      ...
      if (detail && detail.messages) {
        ...
        // is wrong because mapOwlsMessages folds tool_call-only assistant
        ...
        // Update title: use OWLS title, or fallback to first user message
        if (detail.title) {
  ...
  async function switchSessionModel(modelId: string, provider?: string) {
    ...
    // If provider changed, update global config too (OWLS requires it)
    if (provider) {
