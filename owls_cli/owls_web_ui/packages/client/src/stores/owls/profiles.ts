...
import { ref } from 'vue'
import * as profilesApi from '@/api/owls/profiles'
import type { OwlsProfile, OwlsProfileDetail } from '@/api/owls/profiles'

const ACTIVE_PROFILE_STORAGE_KEY = 'owls_active_profile_name'

export const useProfilesStore = defineStore('profiles', () => {
  const profiles = ref<OwlsProfile[]>([])
  ...
  const activeProfile = ref<OwlsProfile | null>(null)
  const detailMap = ref<Record<string, OwlsProfileDetail>>({})
  ...
  function clearProfileCache(profileName: string) {
    const prefixes = [
      `owls_sessions_cache_v1_${profileName}`,
      `owls_session_msgs_v1_${profileName}_`,
      `owls_in_flight_v1_${profileName}_`,
      `owls_active_session_${profileName}`,
      `owls_session_pins_v1_${profileName}`,
      `owls_human_only_v1_${profileName}`,
    ]
