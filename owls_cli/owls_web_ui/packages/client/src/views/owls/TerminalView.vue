...
const STORAGE_KEY_THEME = "owls_terminal_theme";
...
function buildWsUrl(): string {
  ...
  if (base) {
    return `${wsProtocol}//${new URL(base).host}/api/owls/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
  }
  ...
  const host = import.meta.env.DEV
    ...
    : location.host;
  return `${wsProtocol}//${host}/api/owls/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
}
