...
const WEBUI_LOG_FILE = join(homedir(), '.owls-web-ui', 'logs', 'server.log')
...
export async function list(ctx: any) {
  const files = await owlsCli.listLogFiles()
...
export async function read(ctx: any) {
  ...
  try {
    const content = await owlsCli.readLogs(logName, lines, level, session, since)
    const rawLines = content.split('\n')
