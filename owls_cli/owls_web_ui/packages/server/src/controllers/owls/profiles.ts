...
export async function list(ctx: any) {
  try {
    const profiles = await owlsCli.listProfiles()
    ctx.body = { profiles }
  } catch (err: any) {
    ctx.status = 500
...
export async function create(ctx: any) {
  ...
  try {
    const output = await owlsCli.createProfile(name, clone)
...
export async function get(ctx: any) {
  try {
    const profile = await owlsCli.getProfile(ctx.params.name)
    ctx.body = { profile }
  } catch (err: any) {
...
export async function remove(ctx: any) {
  ...
  try {
    ...
    const ok = await owlsCli.deleteProfile(name)
    if (ok) {
...
export async function rename(ctx: any) {
  ...
  try {
    const ok = await owlsCli.renameProfile(ctx.params.name, new_name)
    if (ok) {
      ctx.body = { success: true }
    } else {
      ctx.status = 500
...
export async function exportProfile(ctx: any) {
  const { name } = ctx.params
  const outputPath = join(tmpdir(), `owls-profile-${name}.tar.gz`)
  try {
    await owlsCli.exportProfile(name, outputPath)
...
export async function importProfile(ctx: any) {
  ...
  const tmpDir = join(tmpdir(), 'owls-import')
  ...
  try {
    const result = await owlsCli.importProfile(archivePath)
