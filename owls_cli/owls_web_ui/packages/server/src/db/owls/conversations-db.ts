...
import type {
  ...
  ConversationMessage,
  ConversationSummary,
} from '../../services/owls/conversations'
...
function buildConversationSessionSql(source?: string, userId?: string): { sql: string, params: any[] } {
...
async function loadConversationSessions(source?: string, userId?: string): Promise<ConversationSessionRow[]> {
  const db = await openConversationDb()
  try {
    const { sql, params } = buildConversationSessionSql(source, userId)
...
export async function listConversationSummariesFromDb(options: ConversationListOptions = {}): Promise<ConversationSummary[]> {
  ...
  const sessions = await loadConversationSessions(options.source, options.userId)
