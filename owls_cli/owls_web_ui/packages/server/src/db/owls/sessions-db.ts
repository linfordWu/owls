import { getActiveProfileDir } from '../../services/owls/owls-profile'
...
export interface OWLSSessionRow {
  id: string
  source: string
  user_id: string | null
  model: string
...
export interface OWLSSessionSearchRow extends OWLSSessionRow {
...
function mapRow(row: Record<string, unknown>): OWLSSessionRow {
...
function buildBaseSessionSql(source?: string, userId?: string): { sql: string, params: any[] } {
  ...
  const sql = `SELECT ${SESSION_SELECT}${SESSION_FROM}${filters.length ? `\n    AND ${filters.join('\n    AND ')}` : ''}`
  return { sql, params }
}

function buildListSessionSql(source?: string, limit = 2000, userId?: string): { sql: string, params: any[] } {
  const base = buildBaseSessionSql(source, userId)
  return {
...
function runLikeContentSearch(
  ...
  const likeBase = buildBaseSessionSql(source, userId)
  const likeSql = `
    WITH base AS (
      ${likeBase.sql}
    )
...
function mapSearchRow(row: Record<string, unknown>): OWLSSessionSearchRow {
  return {
    ...mapRow(row),
...
export async function listSessionSummaries(source?: string, limit = 2000, userId?: string): Promise<OWLSSessionRow[]> {
  if (!SQLITE_AVAILABLE) {
  ...
  try {
    const { sql, params } = buildListSessionSql(source, limit, userId)
...
export async function searchSessionSummaries(
  ...
  if (!trimmed) {
    const recent = await listSessionSummaries(source, limit, userId)
  ...
  try {
    const titleBase = buildBaseSessionSql(source, userId)
    const contentBase = buildBaseSessionSql(source, userId)
    ...
    const merged = new Map<string, OWLSSessionSearchRow>()
    ...
    if (containsCjk(normalized)) {
      const likeRows = runLikeContentSearch(db, source, trimmed, userId)
      const merged = new Map<string, OWLSSessionSearchRow>()
...
export async function getSessionOwner(sessionId: string): Promise<string | null | undefined> {
  if (!SQLITE_AVAILABLE) {
    throw new Error(`node:sqlite requires Node >= 22.5, current: ${process.versions.node}`)
  }
  ...
  try {
    const row = db.prepare('SELECT user_id FROM sessions WHERE id = ?').get(sessionId) as Record<string, unknown> | undefined
    if (!row) return undefined
    return normalizeNullableString(row.user_id)
  } finally {
    db.close()
  }
}
