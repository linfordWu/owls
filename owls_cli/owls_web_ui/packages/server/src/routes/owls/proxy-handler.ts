...
import { updateUsage } from '../../db/owls/usage-store'
...
function resolveProfile(ctx: Context): string {
  return ctx.get('x-owls-profile') || (ctx.query.profile as string) || 'default'
}
...
function buildProxyHeaders(ctx: Context, upstream: string): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(ctx.headers)) {
    ...
    } else if (lower === 'origin' || lower === 'referer' || lower === 'connection' || lower === 'authorization' || lower === 'content-length') {
      continue
    } else {
  ...
  if (authUser?.username) {
    headers['x-owls-user-id'] = String(authUser.username)
  }
...
export async function proxy(ctx: Context) {
  ...
  const upstreamPath = ctx.path.replace(/^\/api\/owls\/v1/, '/v1').replace(/^\/api\/owls/, '/api')
