...
import { getActiveProfileDir, getActiveConfigPath, getActiveEnvPath, getActiveAuthPath } from './owls/owls-profile'
import { logger } from './logger'

// --- Provider env var mapping (from owls providers.py OWLS_OVERLAYS + config.py) ---
...
export function extractDescription(content: string): string {
  ...
  if (typeof frontmatterDescription === 'string' && frontmatterDescription.trim()) {
    return frontmatterDescription.replace(/\s+/g, ' ').trim().slice(0, 160)
  }
...
function splitMarkdownFrontmatter(content: string): { frontmatter: Record<string, any>; body: string } {
  ...
  try {
    ...
    // Fall through to body-only extraction when frontmatter is malformed.
  }
...
export const getOwlsDir = () => getActiveProfileDir()
