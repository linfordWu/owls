import { exportSessionsRaw, type OwlsSessionFull } from './owls-cli'
...
type ConversationSession = OwlsSessionFull & {
...
function visibleHumanMessage(message: OwlsMessageLike): boolean {
...
function firstVisibleHumanText(messages: OwlsMessageLike[]): string {
  const firstVisible = messages.find(visibleHumanMessage)
  return firstVisible ? textFromContent(firstVisible.content).trim() : ''
}

function maxMessageTimestamp(messages: OwlsMessageLike[]): number {
...
function enrichSession(session: OwlsSessionFull, nowSeconds: number): ConversationSession {
...
function sessionMessages(session: OwlsSessionFull): OwlsMessageLike[] {
  return Array.isArray(session.messages) ? session.messages as OwlsMessageLike[] : []
}

function normalizeVisibleMessage(message: OwlsMessageLike, session: OwlsSessionFull, index: number): ConversationMessage | null {
...
function visibleMessagesForSessions(sessions: OwlsSessionFull[]): ConversationMessage[] {
  return sessions
...
function hasVisibleHumanMessages(sessions: OwlsSessionFull[]): boolean {
...
export async function listConversationSummaries(options: ConversationListOptions = {}): Promise<ConversationSummary[]> {
  ...
  const sessions = (await loadSessions(options.source))
    .filter(session => !options.userId || session.user_id === options.userId)
