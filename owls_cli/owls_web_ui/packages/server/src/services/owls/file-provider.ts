...
import { getActiveProfileDir, getActiveEnvPath } from './owls-profile'
...
export interface FileEntry {
  name: string
  path: string       // relative to owls home
  isDir: boolean
  size: number
...
/**
 * Resolve a relative path to an absolute path under the owls home directory.
 * Validates path safety (no traversal).
 */
export function resolveOWLSPath(relativePath: string): string {
...
/**
 * Read terminal config from owls config.yaml.
 */
...
/**
 * Read SSH env vars from owls .env file.
 */
...
async function resolveDockerContainer(cfg: TerminalConfig): Promise<string> {
  ...
  throw Object.assign(
    new Error('Cannot determine Docker container. Set terminal.docker_container_name in owls config.'),
    { code: 'backend_error' },
  )
}
...
/**
 * Create a FileProvider based on the active owls terminal config.
