 ...
 *
 * 启动模式：
 *   - Linux systemd：owls gateway start/stop（系统服务管理）
 *   - Docker：owls gateway run（detached 子进程，手动 kill）
 */
...
const OWLS_BASE = resolve(homedir(), '.owls')
const OWLS_BIN = process.env.OWLS_BIN?.trim() || 'owls'

// Docker 没有 systemd，需要用 "gateway run" 代替 "gateway start"
const isDocker = existsSync('/.dockerenv')
const needsRunMode = isDocker
...
export class GatewayManager {
  ...
  private profileDir(name: string): string {
    if (name === 'default') return OWLS_BASE
    return join(OWLS_BASE, 'profiles', name)
  }
   ...
   *         port: <port>
   *         host: <host>
   * 同时清理旧的顶层 port/host（避免 OWLS 读取错误）
   */
  private writeProfilePort(name: string, port: number, host: string): void {
    ...
    try {
      ...
      // 清理旧的顶层 port/host，OWLS 只从 extra 读取
  ...
  /** 列出所有已知 profile 名称（通过 owls CLI 或文件系统扫描） */
  async listProfiles(): Promise<string[]> {
    try {
      ...
      const profilesDir = join(OWLS_BASE, 'profiles')
  ...
  async start(name: string): Promise<GatewayStatus> {
    ...
    const owlsHome = this.profileDir(name)
    const url = `http://${host}:${port}`

    if (needsRunMode) {
      // Docker：无 systemd，用 "gateway run" 作为 detached 子进程
      return new Promise((resolve, reject) => {
        const env = { ...process.env, OWLS_HOME: owlsHome }
        const child = spawn(OWLS_BIN, ['gateway', 'run', '--replace'], {
          detached: true,
          stdio: 'ignore',
          windowsHide: true,
          env,
    ...
    const env = { ...process.env, OWLS_HOME: owlsHome }
    try {
      const { stdout } = await execFileAsync(OWLS_BIN, ['gateway', 'start'], {
        timeout: 30000,
        env,
        windowsHide: true,
      })
      ...
      try {
        const { stdout } = await execFileAsync(OWLS_BIN, ['gateway', 'restart'], {
          timeout: 30000,
          env,
          windowsHide: true,
        })
  ...
  /**
   * 停止单个 profile 的网关
   * 正常系统用 "gateway stop"，Docker 直接 kill 进程组
  ...
  async stop(name: string, timeoutMs = 10000): Promise<void> {
    ...
    if (!needsRunMode) {
      // 正常系统：通过 owls CLI 停止系统服务
      try {
        const owlsHome = this.profileDir(name)
        const env = { ...process.env, OWLS_HOME: owlsHome }
        await execFileAsync(OWLS_BIN, ['gateway', 'stop'], {
          timeout: 10000,
          env,
          windowsHide: true,
        })
