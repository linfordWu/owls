...
const MODELS_DEV_CACHE = resolve(OWLS_BASE, 'models_dev_cache.json')
...
function getProfileDir(profile?: string): string {
  if (!profile || profile === 'default') return OWLS_BASE
  const dir = join(OWLS_BASE, 'profiles', profile)
  return existsSync(dir) ? dir : OWLS_BASE
}
