...
function resolveOwlsBin(): string {
  const envBin = process.env.OWLS_BIN?.trim()
  if (envBin) return envBin
  return 'owls'
}

const OWLS_BIN = resolveOwlsBin()

export interface OwlsSession {
  id: string
  source: string
  user_id: string | null
  model: string
...
export interface OwlsSessionFull {
  id: string
  source: string
  user_id: string | null
  model: string
...
function parseSessionExport(stdout: string): OwlsSessionFull[] {
  const lines = stdout.trim().split('\n').filter(Boolean)
  const sessions: OwlsSessionFull[] = []
  for (const line of lines) {
    try {
      const raw: OwlsSessionFull = JSON.parse(line)
      sessions.push(raw)
    } catch {
...
export async function exportSessionsRaw(source?: string): Promise<OwlsSessionFull[]> {
  const args = ['sessions', 'export', '-']
  if (source) args.push('--source', source)

  try {
    const { stdout } = await execFileAsync(OWLS_BIN, args, {
    ...
    logger.error(err, 'OWLS CLI: sessions export failed')
    throw new Error(`Failed to list sessions: ${err.message}`)
  }
}

/**
 * List sessions from OWLS CLI (without messages)
 */
export async function listSessions(source?: string, limit?: number): Promise<OwlsSession[]> {
  const raws = await exportSessionsRaw(source)
  const sessions: OwlsSession[] = []
...
/**
 * Get a single session with messages from OWLS CLI
 */
export async function getSession(id: string): Promise<OwlsSession | null> {
  ...
  try {
    ...
    const raw: OwlsSessionFull = raws[0]
    return {
      id: raw.id,
      source: raw.source,
      user_id: raw.user_id,
    ...
    logger.error(err, 'OWLS CLI: session export failed')
    throw new Error(`Failed to get session: ${err.message}`)
  }
}

/**
 * Delete a session from OWLS CLI
 */
export async function deleteSession(id: string): Promise<boolean> {
  try {
    await execFileAsync(OWLS_BIN, ['sessions', 'delete', id, '--yes'], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'OWLS CLI: session delete failed')
    return false
  }
}

/**
 * Rename a session title via OWLS CLI
 */
export async function renameSession(id: string, title: string): Promise<boolean> {
  try {
    await execFileAsync(OWLS_BIN, ['sessions', 'rename', id, title], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'OWLS CLI: session rename failed')
    return false
  }
}
...
export async function getVersion(): Promise<string> {
  try {
    const { stdout } = await execFileAsync(OWLS_BIN, ['--version'], { timeout: 5000, ...execOpts })
    return stdout.trim()
  } catch {
    return ''
  }
...
/**
 * Start OWLS gateway through Linux systemd when available.
 */
export async function startGateway(): Promise<string> {
  ...
  const { stdout, stderr } = await execFileAsync(OWLS_BIN, ['gateway', 'start'], {
    timeout: 30000,
    ...execOpts,
  })
...
/**
 * Start OWLS gateway in background when systemd is unavailable.
 * Uses "owls gateway run" as a detached background process
 */
export async function startGatewayBackground(): Promise<number | null> {
  const child = spawn(OWLS_BIN, ['gateway', 'run'], {
    detached: true,
    stdio: 'ignore',
  })
  child.unref()
...
export async function restartGateway(): Promise<string> {
  ...
  const { stdout, stderr } = await execFileAsync(OWLS_BIN, ['gateway', 'restart'], {
    timeout: 30000,
    ...execOpts,
  })
  return stdout || stderr
...
export async function stopGateway(): Promise<string> {
  const { stdout, stderr } = await execFileAsync(OWLS_BIN, ['gateway', 'stop'], {
    timeout: 30000,
    ...execOpts,
  })
  return stdout || stderr
...
export async function listLogFiles(): Promise<LogFileInfo[]> {
  try {
    const { stdout } = await execFileAsync(OWLS_BIN, ['logs', 'list'], {
      timeout: 10000,
      ...execOpts,
    })
    ...
    logger.error(err, 'OWLS CLI: logs list failed')
    return []
  }
}
...
export async function readLogs(
  ...
  try {
    ...
    logger.error(err, 'OWLS CLI: logs read failed')
 ...
 * List all profiles
 */
export async function listProfiles(): Promise<OwlsProfile[]> {
  try {
    const { stdout } = await execFileAsync(OWLS_BIN, ['profile', 'list'], {
      timeout: 10000,
      ...execOpts,
    })

    const lines = stdout.trim().split('\n').filter(Boolean)
    const profiles: OwlsProfile[] = []
    ...
    logger.error(err, 'OWLS CLI: profile list failed')
 ...
 * Get profile details
 */
export async function getProfile(name: string): Promise<OwlsProfileDetail> {
  try {
    const { stdout } = await execFileAsync(OWLS_BIN, ['profile', 'show', name], {
      timeout: 10000,
      ...execOpts,
    })
    ...
    logger.error(err, 'OWLS CLI: profile show failed')
...
export async function createProfile(name: string, clone?: boolean): Promise<string> {
  ...
  try {
    const { stdout, stderr } = await execFileAsync(OWLS_BIN, args, {
      timeout: 15000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    logger.error(err, 'OWLS CLI: profile create failed')
...
export async function deleteProfile(name: string): Promise<boolean> {
  try {
    await execFileAsync(OWLS_BIN, ['profile', 'delete', name, '--yes'], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'OWLS CLI: profile delete failed')
    return false
  }
}
...
export async function renameProfile(oldName: string, newName: string): Promise<boolean> {
  try {
    await execFileAsync(OWLS_BIN, ['profile', 'rename', oldName, newName], {
      timeout: 10000,
      ...execOpts,
    })
    return true
  } catch (err: any) {
    logger.error(err, 'OWLS CLI: profile rename failed')
    return false
  }
}
...
export async function useProfile(name: string): Promise<string> {
  try {
    const { stdout, stderr } = await execFileAsync(OWLS_BIN, ['profile', 'use', name], {
      timeout: 10000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    logger.error(err, 'OWLS CLI: profile use failed')
...
export async function exportProfile(name: string, outputPath?: string): Promise<string> {
  ...
  try {
    ...
    logger.error(err, 'OWLS CLI: profile export failed')
    throw new Error(`Failed to export profile: ${err.message}`)
  }
}

/**
 * Run owls setup --non-interactive --reset to generate default config for current profile
 */
export async function setupReset(): Promise<string> {
  try {
    const { stdout, stderr } = await execFileAsync(OWLS_BIN, ['setup', '--non-interactive', '--reset'], {
      timeout: 30000,
      ...execOpts,
    })
    return stdout || stderr
  } catch (err: any) {
    logger.error(err, 'OWLS CLI: setup reset failed')
...
export async function importProfile(archivePath: string, name?: string): Promise<string> {
  ...
  try {
    ...
    logger.error(err, 'OWLS CLI: profile import failed')
