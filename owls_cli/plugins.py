...
1. **User plugins**   – ``~/.owls/plugins/<name>/``
2. **Project plugins** – ``./.owls/plugins/<name>/`` (opt-in via
   ``OWLS_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``owls.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "owls.plugins"

_NS_PARENT = "owls_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``owls honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``owls <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.owls/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.owls/plugins/)
        user_dir = get_owls_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.owls/plugins/)
        if _env_enabled("OWLS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".owls" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``owls_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``owls tools`` TUI so plugin-provided toolsets appear
