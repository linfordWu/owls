"""
Profile management for multiple isolated OWLS instances.

Each profile is a fully independent OWLS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.owls/profiles/<name>/`` by default.

The "default" profile is ``~/.owls`` itself — backward compatible,
zero migration needed.

Usage::

    owls profile create coder          # fresh profile + bundled skills
    owls profile create coder --clone  # also copy config, .env, SOUL.md
    owls profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    owls -p coder chat                 # or via flag
    owls profile use coder             # set as sticky default
    owls profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See owls_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.owls) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "owls",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "owls", "default", "test", "tmp", "root", "sudo",
})

# OWLS subcommands that cannot be used as profile names/aliases
_OWLS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the owls root, NOT to the current OWLS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where OWLS_HOME points outside
    ``~/.owls``, profiles live under ``OWLS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_owls_home() / "profiles"


def _get_default_owls_home() -> Path:
    """Return the default (pre-profile) OWLS_HOME path.

    In standard deployments this is ``~/.owls``.
    In Docker/custom deployments where OWLS_HOME is outside ``~/.owls``
    (e.g. ``/opt/data``), returns OWLS_HOME directly.
    """
    from owls_constants import get_default_owls_root
    return get_default_owls_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_owls_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its OWLS_HOME directory."""
    if name == "default":
        return _get_default_owls_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, owls subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _OWLS_SUBCOMMANDS:
        return f"'{name}' conflicts with a owls subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_owls_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.owls)."
        )
    ...
    if not soul_path.exists():
        try:
            from owls_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches OWLS_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OWLS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.owls).\n"
            "To remove everything, use: owls uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set OWLS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("OWLS_HOME")
    try:
        os.environ["OWLS_HOME"] = str(profile_dir)
        from owls_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["OWLS_HOME"] = old_home
        elif "OWLS_HOME" in os.environ:
            del os.environ["OWLS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.owls/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: owls profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from OWLS_HOME.

    Returns ``"default"`` if OWLS_HOME is not set or points to ``~/.owls``.
    Returns the profile name if OWLS_HOME points into ``~/.owls/profiles/<name>``.
    Returns ``"custom"`` if OWLS_HOME is set to an unrecognized path.
    """
    from owls_constants import get_owls_home
    owls_home = get_owls_home()
    resolved = owls_home.resolve()

    default_resolved = _get_default_owls_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.owls itself — its parent is ~/ and its
        # directory name is ".owls", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: owls profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.owls itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.owls). "
            "Specify a different name: owls profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for owls profile names."""
    return '''# OWLS profile completion
# Add to ~/.bashrc: eval "$(owls completion bash)"

_owls_profiles() {
    local profiles_dir="$HOME/.owls/profiles"
...
_owls_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_owls_profiles)" -- "$cur"))
        return
    fi
...
complete -F _owls_completion owls
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for owls profile names."""
    return '''#compdef owls
# OWLS profile completion
# Add to ~/.zshrc: eval "$(owls completion zsh)"

_owls() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.owls/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.owls/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a OWLS_HOME path string.

    Called early in the CLI entry point, before any owls modules
    are imported, to set the OWLS_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: owls profile create {profile_name}"
        )
