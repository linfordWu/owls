"""
Single source of truth for provider identity in OWLS.
...
2. **OWLS overlays** — transport type, auth patterns, aggregator flags,
...
# -- OWLS overlay ----------------------------------------------------------
# OWLS-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class OWLSOverlay:
    """OWLS-specific provider metadata layered on top of models.dev."""
...
OWLS_OVERLAYS: Dict[str, OWLSOverlay] = {
    "openrouter": OWLSOverlay(
    ...
    "openai-codex": OWLSOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": OWLSOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="OWLS_QWEN_BASE_URL",
    ),
    "google-gemini-cli": OWLSOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "copilot-acp": OWLSOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": OWLSOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": OWLSOverlay(
    ...
    "minimax-cn": OWLSOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": OWLSOverlay(
        transport="openai_chat",
    ...
    "opencode": OWLSOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": OWLSOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "owls", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. OWLS overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + OWLS overlay
    ...
    overlay = OWLS_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + owls extra
    ...
    if overlay is not None:
        # OWLS-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in OWLS_OVERLAYS
    if provider == "bedrock":
