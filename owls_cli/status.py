"""
Status command for owls CLI.

Shows the status of all OWLS components.
"""
...
from owls_cli.auth import AuthError, resolve_provider
from owls_cli.colors import Colors, color
from owls_cli.config import get_env_path, get_env_value, get_owls_home, load_config
from owls_cli.models import provider_label
from owls_cli.nous_subscription import get_nous_subscription_features
from owls_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all OWLS components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ OWLS Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from owls_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: owls model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: owls model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: owls model)"
    ...
    if not terminal_env:
        ...
        # (owls status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    try:
        from owls_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_owls_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_owls_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'owls doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'owls setup' to configure", Colors.DIM))
    print()
