...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or owls -c.",
    ...
    "Ctrl+Z suspends OWLS to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "owls -c resumes your most recent CLI session. owls -c \"project name\" resumes by title.",
    "owls -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "owls -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "owls chat -t web,terminal enables only specific toolsets for a focused session.",
    "owls chat -s github-pr-workflow preloads a skill at launch.",
    "owls chat -q \"query\" runs a single non-interactive query and exits.",
    "owls chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "owls chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "owls --yolo bypasses all dangerous command approval prompts for the entire session.",
    "owls chat --source telegram tags the session for filtering in owls sessions list.",
    "owls -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "owls doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "owls dump outputs a compact setup summary — great for bug reports.",
    "owls config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "owls config edit opens config.yaml in your default editor.",
    "owls config check scans for missing or stale configuration options.",
    "owls sessions browse opens an interactive session picker with search.",
    "owls sessions stats shows session counts by platform and database size.",
    "owls sessions prune --older-than 30 cleans up old sessions.",
    "owls skills search react --source skills-sh searches the skills.sh public directory.",
    "owls skills check scans installed hub skills for upstream updates.",
    "owls skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "owls skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "owls mcp add github --command npx adds MCP servers from the command line.",
    "owls mcp serve runs OWLS itself as an MCP server for other agents.",
    "owls auth add lets you add multiple API keys for credential pool rotation.",
    "owls completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "owls logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "owls backup creates a zip backup of your entire OWLS home directory.",
    "owls profile create coder creates an isolated profile that becomes its own command.",
    "owls profile create work --clone copies your current config and keys to a new profile.",
    "owls update syncs new bundled skills to ALL profiles automatically.",
    "owls gateway install sets up OWLS as a system service (systemd/launchd).",
    "owls memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "owls webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as owls -w).",
    ...
    "OWLS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call OWLS tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'owls profile create coder' creates the 'coder' command.",
    "owls profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, OWLS prints a resume command with session ID and stats.",
    "owls sessions export backup.jsonl exports all sessions for backup or analysis.",
    "owls -r SESSION_ID resumes any specific past session by its ID.",
    ...
    # --- Skills ---
    "Bundled skills are installed by default when they can run in a Linux OWLS deployment.",
    "Every installed skill automatically becomes a slash command — type / to see them all.",
    "owls skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .owls/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: owls cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "OWLS runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "owls gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to OWLS via a local macOS server.",
    ...
    "Context files (.owls.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.owls/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "OWLS loads project context from .owls.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — owls tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.owls/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "owls chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "owls plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "owls acp runs OWLS as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "OWLS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "owls login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "owls status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.owls/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.owls/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make OWLS your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.owls/SOUL.md to customize.",
    ...
    "Before a gateway session reset, OWLS auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.owls/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at OWLS_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "OWLS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in OWLS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.owls/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'owls profile create ls' would be rejected.",
    "owls profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "owls claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "OWLS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via owls tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.owls/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.owls/skills/.hub/quarantine/ holds skills pending security review.",
]
