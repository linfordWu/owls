...
- Keep data: Remove code but keep ~/.owls/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove OWLS PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing owls or owls PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# OWLS" comment and following line
                if '# OWLS' in line or '# owls' in line:
                    skip_next = True
                    continue
                if skip_next and ('owls' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing owls
                if 'owls' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the owls wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "owls",
        Path("/usr/local/bin/owls"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains owls_cli reference)
                content = wrapper.read_text()
                if 'owls_cli' in content or 'owls' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``owls gateway run`` processes
    ...
    try:
        from owls_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from owls_cli.gateway import get_launchd_plist_path
...
def _is_default_owls_home(owls_home: Path) -> bool:
    """Return True when ``owls_home`` points at the default (non-profile) root."""
    try:
        from owls_constants import get_default_owls_root
        return owls_home.resolve() == get_default_owls_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its OWLS_HOME directory.

    We shell out to ``owls -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current OWLS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m owls_cli.main` so we don't depend on a `owls`
    #    wrapper that may be half-removed mid-uninstall.
    owls_invocation = [_sys.executable, "-m", "owls_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                owls_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's OWLS_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.owls/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.owls/ for future reinstall
    """
    project_root = get_project_root()
    owls_home = get_owls_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie OWLS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_owls_home(owls_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ OWLS Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {owls_home / 'config.yaml'}")
    print(f"  Secrets: {owls_home / '.env'}")
    print(f"  Data:    {owls_home / 'cron/'}, {owls_home / 'sessions/'}, {owls_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their OWLS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL OWLS data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the OWLS code but keep your configuration and data.")
    ...
    log_info("Removing owls command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.owls/, just remove the owls subdir
            if owls_home in project_root.parents or project_root.parent == owls_home:
    ...
    # 5. Optionally remove ~/.owls/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile OWLS_HOME dirs live under
        ...
        try:
            if owls_home.exists():
                shutil.rmtree(owls_home)
                log_success(f"Removed {owls_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {owls_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {owls_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/owls/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using OWLS! ⚕")
    print()
