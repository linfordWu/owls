"""owls webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    owls webhook subscribe <name> [options]
    owls webhook list
    owls webhook remove <name>
    owls webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.owls/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _owls_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: owls gateway run
"""
...
def webhook_command(args):
    """Entry point for 'owls webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: owls webhook {subscribe|list|remove|test}")
        print("Run 'owls webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (owls gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: owls webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from owls webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (owls gateway run)")
