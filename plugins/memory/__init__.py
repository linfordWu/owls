...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with owls)
2. User-installed providers: ``$OWLS_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$OWLS_HOME/plugins/`` or None if unavailable."""
    try:
        from owls_constants import get_owls_home
        d = get_owls_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$OWLS_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_owls_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_owls_user_memory.{active_provider}.cli"
    try:
