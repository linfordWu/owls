...
```bash
owls memory setup    # select "byterover"
```

Or manually:
```bash
owls config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.owls/.env
```
...
Working directory: `$OWLS_HOME/byterover/` (profile-scoped).
