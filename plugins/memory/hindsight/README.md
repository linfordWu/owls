...
```bash
owls memory setup    # select "hindsight"
```
...
```bash
owls config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.owls/.env
```
...
OWLS spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.owls/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p owls ui start
```
...
Config file: `~/.owls/hindsight/config.json`
...
| `retain_context` | `conversation between OWLS and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.owls/.env` as `HINDSIGHT_LLM_API_KEY`.
