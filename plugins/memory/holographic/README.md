...
```bash
owls memory setup    # select "holographic"
```

Or manually:
```bash
owls config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.owls-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$OWLS_HOME/memory_store.db` | SQLite database path |
