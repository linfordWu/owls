...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/owls>
...
```bash
owls honcho setup    # full interactive wizard (cloud or local)
owls memory setup    # generic picker, also works
```

Or manually:
```bash
owls config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.owls/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$OWLS_HOME/honcho.json` | Profile-local (isolated OWLS instances) |
| 2 | `~/.owls/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active OWLS profile: `owls` (default) or `owls.<profile>`.
...
| 4 | `per-session` strategy | OWLS session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`owls`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`owls`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-owls`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening owls in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — OWLS session ID (timestamp + hex). Every `owls` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple OWLS profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`owls`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active OWLS profile: `owls` (default) or `owls.<profile>` (e.g. `owls -p coder` → host key `owls.coder`).
...
| `OWLS_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `owls honcho setup` | Full interactive setup wizard |
| `owls honcho status` | Show resolved config for active profile |
| `owls honcho enable` / `disable` | Toggle Honcho for active profile |
| `owls honcho mode <mode>` | Change recall or observation mode |
| `owls honcho peer --user <name>` | Update user peer name |
| `owls honcho peer --ai <name>` | Update AI peer name |
| `owls honcho tokens --context <N>` | Set context token budget |
| `owls honcho tokens --dialectic <N>` | Set dialectic max chars |
| `owls honcho map <name>` | Map current directory to a session name |
| `owls honcho sync` | Create host blocks for all OWLS profiles |
