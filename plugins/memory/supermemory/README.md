...
```bash
owls memory setup    # select "supermemory"
```

Or manually:

```bash
owls config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.owls/.env
```

## Config

Config file: `$OWLS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `owls` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `owls-{identity}` → `owls-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per OWLS profile:

```json
{
  "container_tag": "owls-{identity}"
}
```

For a profile named `coder`, this resolves to `owls-coder`. The default profile resolves to `owls-default`. Without `{identity}`, all profiles share the same container.
