...
def _load_supermemory_config(owls_home: str) -> dict:
    config = _default_config()
    config_path = Path(owls_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, owls_home: str) -> None:
    config_path = Path(owls_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `owls memory setup`.
        # All other options are documented for $OWLS_HOME/supermemory.json
    ...
    def save_config(self, values, owls_home):
        ...
        _save_supermemory_config(sanitized, owls_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from owls_constants import get_owls_home
        self._owls_home = kwargs.get("owls_home") or str(get_owls_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._owls_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "owls", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "owls_memory", "target": target, "type": "explicit_memory"},
