...
from owls_cli.timeouts import get_provider_request_timeout

_owls_home = get_owls_home()
_project_env = Path(__file__).parent / '.env'
_loaded_env_paths = load_owls_dotenv(owls_home=_owls_home, project_env=_project_env)
...
# Agent team: planner, experience, reflection
from agent.planner import Planner, is_complex_task
...
from agent.prompt_builder import (
    ...
    SECURITY_GUIDANCE, TODO_EXECUTION_GUIDANCE,
)
...
class _SafeWriter:
    ...
    When owls runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        # both live under ~/.owls/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from owls_logging import setup_logging, setup_verbose_logging
        setup_logging(owls_home=_owls_home)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from owls_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://owls.nousresearch.com",
                        "X-OpenRouter-Title": "OWLS",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif "api.githubcopilot.com" in effective_base.lower():
                    from owls_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `owls model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `owls model` to "
                        "select a provider, or run `owls setup` for first-time "
                        "configuration."
                    )
        ...
        # Session logs go into ~/.owls/sessions/ alongside gateway sessions
        owls_home = get_owls_home()
        self.logs_dir = owls_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("OWLS_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from owls_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from owls_constants import get_owls_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "owls"
                        except Exception:
                            pass
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from owls_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by OWLS.  Choose a model with at least "
        ...
        # Planner / experience / reflection integration (competition feature)
        # Read from config; default off so existing behavior is unchanged.
        _agent_section = _agent_cfg.get("agent", {}) if isinstance(_agent_cfg, dict) else {}
        self._planner_enabled = str(_agent_section.get("planner_enabled", False)).lower() in ("true", "1", "yes", "on")
        self._experience_enabled = str(_agent_section.get("experience_enabled", False)).lower() in ("true", "1", "yes", "on")
        self._in_plan_execution = False
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        import logging
        import re as _re
        from owls_cli.providers import determine_api_mode
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``owls chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `owls setup` or set OPENROUTER_API_KEY."
                )
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``OWLS_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``OWLS_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("OWLS_API_TIMEOUT", 1800.0))
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from owls_cli.models import _should_use_copilot_responses_api
    ...
    def _spawn_background_review(
        ...
        def _run_review():
            ...
            try:
                ...
                for msg in getattr(review_agent, "_session_messages", []):
                    ...
                    if "created" in message.lower() or "updated" in message.lower():
                        if "skill" in message.lower():
                            skill_actions.append(message)
                        else:
                            memory_actions.append(message)
                    elif "added" in message.lower() or (target and "add" in message.lower()):
                        label = "Memory" if target == "memory" else "User profile" if target == "user" else target
                        memory_actions.append(f"{label} updated")
                ...
                # Explicit notice for auto-generated skills — hard to miss
                if skill_actions:
                    skill_summary = "  ·  ".join(dict.fromkeys(skill_actions))
                    notice_lines = [
                        "",
                        "┌─────────────────────────────────────────────────────────────┐",
                        "│  🤖  Auto-generated Skill Notice                             │",
                        "├─────────────────────────────────────────────────────────────┤",
                        f"│  {skill_summary:<57} │",
                        "│                                                             │",
                        "│  The system reviewed your conversation and saved the        │",
                        "│  approach as a reusable skill.                              │",
                        "│  Location: ~/.owls/skills/                               │",
                        "│  You can edit or delete it with: owls skills             │",
                        "└─────────────────────────────────────────────────────────────┘",
                        "",
                    ]
                    ...
                    if _bg_cb:
                        try:
                            _bg_cb(f"🤖 Auto-generated skill: {skill_summary}")
                        except Exception:
                            pass

                if memory_actions:
                    mem_summary = " · ".join(dict.fromkeys(memory_actions))
                    self._safe_print(f"  💾 {mem_summary}")
                    _bg_cb = self.background_review_callback
                    if _bg_cb:
                        try:
                            _bg_cb(f"💾 {mem_summary}")
                        except Exception:
                            pass
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("OWLS_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Security guidance: always inject for terminal AI applications
        prompt_parts.append(SECURITY_GUIDANCE)

        # Responsible task execution: todolist before complex operations
        prompt_parts.append(TODO_EXECUTION_GUIDANCE)
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the owls install
        ...
        from owls_time import now as _owls_now
        now = _owls_now()
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from owls_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("OWLS_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("OWLS_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        _stale_base = float(os.getenv("OWLS_API_CALL_STALE_TIMEOUT", 300.0))
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # wins over the OWLS_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("OWLS_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # OWLS_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("OWLS_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode OWLS_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("OWLS_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("OWLS_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set OWLS_STREAM_STALE_TIMEOUT.
    ...
    def _try_activate_fallback(self) -> bool:
        ...
        try:
            ...
            try:
                from owls_cli.model_normalize import normalize_model_for_provider
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self._is_qwen_portal():
            api_kwargs["metadata"] = {
                "sessionId": self.session_id or "owls",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from owls_cli.models import github_model_reasoning_efforts
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        try:
            from owls_cli.plugins import get_pre_tool_call_block_message
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=owls"]
    ...
    def run_conversation(
        ...
        # ``owls logs --session <id>`` can filter a single conversation.
        from owls_logging import set_session_context
        ...
        # ── Complex-task planning (competition integration) ──
        # Only on first turns (no history) and when planner is enabled.
        # Nested plan execution is skipped to avoid infinite recursion.
        if (
            self._planner_enabled
            and not self._in_plan_execution
            and not conversation_history
            and is_complex_task(user_message)
        ):
            try:
                planner = Planner()
                plan = planner.generate_plan(user_message, self.tools or [])
                if plan.nodes:
                    if not self.quiet_mode:
                        self._safe_print(
                            f"📋 Complex task detected — executing plan with {len(plan.nodes)} node(s)"
                        )
                    self._in_plan_execution = True
                    try:
                        plan_result = plan.execute(self)
                    finally:
                        self._in_plan_execution = False
                    # Summarise plan result as the final response
                    _plan_summary = (
                        f"Plan execution complete.\n"
                        f"Successes: {len(plan_result.successes)}\n"
                        f"Failures: {len(plan_result.failures)}\n"
                        f"Rolled back: {len(plan_result.rolled_back)}"
                    )
                    if plan_result.failures:
                        _plan_summary += (
                            "\n\nFailed nodes: " + ", ".join(plan_result.failures)
                        )
                    messages.append({"role": "assistant", "content": _plan_summary})
                    return {
                        "final_response": _plan_summary,
                        "messages": messages,
                        "api_calls": 0,
                        "completed": plan_result.completed and not plan_result.failures,
                        "partial": bool(plan_result.failures),
                        "interrupted": False,
                        "model": self.model,
                        "provider": self.provider,
                        "base_url": self.base_url,
                ...
                logger.warning("Planner execution failed, falling back to normal conversation: %s", exc)
        ...
        if self._cached_system_prompt is None:
            ...
            if stored_prompt:
                ...
                # Load relevant prompt fragments from reflection engine
                # (keyword match against user_message; must happen before
                # first non-system message to preserve prompt caching.)
                if user_message and self._experience_enabled:
                    try:
                        if self._reflection_engine is None:
                            self._reflection_engine = ReflectionEngine()
                        fragments = self._reflection_engine.load_relevant_fragments(
                        ...
                        logger.debug("Failed to load prompt fragments: %s", exc)
        ...
        # are reused.  The system prompt is OWLS's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for OWLS internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("OWLS_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for OWLS-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: owls config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: owls config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `owls auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: owls setup", force=True)
        ...
        # ── Experience recording & reflection (competition integration) ──
        # Record the turn outcome and optionally generate a reflection.
        # Runs at the very end so it never blocks response delivery.
        if self._experience_enabled and original_user_message:
            try:
                ...
                # Derive a simple plan summary from the conversation
                _tool_sequence = []
                for m in messages:
                ...
                _plan_summary = "conversation" if not _tool_sequence else f"tools: {', '.join(_tool_sequence[:10])}"
                _failure = result.get("error") or ("interrupted" if interrupted else None)
                _success = completed and not bool(_failure)

                import time as _time
                _exec_time_ms = int((_time.time() - _time.time()) * 1000) or 1
                # Use a simple wall-clock estimate: we don't track start time
                # precisely, so approximate from api_call_count (~2s each).
                _exec_time_ms = max(api_call_count * 2000, 1)

                self._experience_store.record(
                    session_id=self.session_id or "unknown",
                    input_msg=original_user_message,
                ...
                # Reflection is best-effort and runs in background so it
                ...
                try:
                    self._reflection_engine.reflect(_exp)
                except Exception:
                    pass
            except Exception as exc:
