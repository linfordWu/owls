#!/usr/bin/env bash
# Canonical test runner for owls. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.owls/owls/venv"; do
...
# Unset OWLS_* behavioral vars too.
unset OWLS_YOLO_MODE OWLS_INTERACTIVE OWLS_QUIET OWLS_TOOL_PROGRESS \
      OWLS_TOOL_PROGRESS_MODE OWLS_MAX_ITERATIONS OWLS_SESSION_PLATFORM \
      OWLS_SESSION_CHAT_ID OWLS_SESSION_CHAT_NAME OWLS_SESSION_THREAD_ID \
      OWLS_SESSION_SOURCE OWLS_SESSION_KEY OWLS_GATEWAY_SESSION \
      OWLS_PLATFORM OWLS_INFERENCE_PROVIDER OWLS_MANAGED OWLS_DEV \
      OWLS_CONTAINER OWLS_EPHEMERAL_SYSTEM_PROMPT OWLS_TIMEZONE \
      OWLS_REDACT_SECRETS OWLS_BACKGROUND_NOTIFICATIONS OWLS_EXEC_ASK \
      OWLS_HOME_MODE 2>/dev/null || true
