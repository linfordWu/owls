...
# 5. Symlinks the 'owls' CLI command into a user-facing bin dir
...
# ============================================================================
# PATH setup — symlink owls into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up owls command..."

OWLS_BIN="$SCRIPT_DIR/venv/bin/owls"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$OWLS_BIN" "$COMMAND_LINK_DIR/owls"
echo -e "${GREEN}✓${NC} Symlinked owls → $COMMAND_LINK_DISPLAY_DIR/owls"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# OWLS — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.owls/skills/
# ============================================================================

OWLS_SKILLS_DIR="${OWLS_HOME:-$HOME/.owls}/skills"
mkdir -p "$OWLS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.owls/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$OWLS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  owls status        # Check configuration"
if is_termux; then
    echo "  owls gateway       # Run gateway in foreground"
else
    echo "  owls gateway install # Install gateway service (messaging + cron)"
fi
echo "  owls cron list     # View scheduled jobs"
echo "  owls doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m owls_cli.main setup
fi
