...
- Give it a title like "owls-<machine-name>"
...
elif [ -f ~/.owls/.env ] && grep -q "^GITHUB_TOKEN=" ~/.owls/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.owls/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
