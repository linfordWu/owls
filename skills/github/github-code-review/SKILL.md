...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.owls/.env ] && grep -q "^GITHUB_TOKEN=" ~/.owls/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.owls/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${OWLS_HOME:-$HOME/.owls}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by OWLS. Code looks clean — good test coverage, no security concerns."
