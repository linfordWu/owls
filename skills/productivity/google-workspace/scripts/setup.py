#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for OWLS.
...
try:
    ...
    OWLS_ROOT = Path(__file__).resolve().parents[4]
    if OWLS_ROOT.exists():
        sys.path.insert(0, str(OWLS_ROOT))
    from owls_constants import display_owls_home, get_owls_home

OWLS_HOME = get_owls_home()
TOKEN_PATH = OWLS_HOME / "google_token.json"
CLIENT_SECRET_PATH = OWLS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = OWLS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same OWLS profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to OWLS home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_owls_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for OWLS")
