...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("OWLS_HOME", os.path.expanduser("~/.owls")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("OWLS_HOME", Path.home() / ".owls")) / "skills" / "red-teaming" / "godmode"
...
OWLS_HOME = Path(os.getenv("OWLS_HOME", Path.home() / ".owls"))
CONFIG_PATH = OWLS_HOME / "config.yaml"
PREFILL_PATH = OWLS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/OWLS models — already uncensored, just needs clean prompt
    "owls": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "owls" in model_lower or "nous" in model_lower:
        return "owls"
...
def _get_current_model() -> tuple:
    """Read current model and provider from OWLS config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.owls/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart OWLS for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart OWLS for changes to take effect.")
