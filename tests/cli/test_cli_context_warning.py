...
def _isolate(tmp_path, monkeypatch):
    """Isolate OWLS_HOME so tests don't touch real config."""
    home = tmp_path / ".owls"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal OWLSCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = OWLSCLI.__new__(OWLSCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["owls-core"]
        obj.compact = False
