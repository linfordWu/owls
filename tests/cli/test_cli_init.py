"""Tests for OWLSCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a OWLSCLI instance with minimal mocking."""
    import importlib
    ...
    clean_env = {"LLM_MODEL": "", "OWLS_MAX_ITERATIONS": ""}
    if env_overrides:
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), \
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
            ...
            return _cli_mod.OWLSCLI(**kwargs)
...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"OWLS_MAX_ITERATIONS": "42"})
...
class _FakeCompletionBuffer:
    def __init__(
        ...
        self.complete_state = SimpleNamespace(
            current_completion=SimpleNamespace(
    ...
    def go_to_completion(self, index: int):
        return None

    def apply_completion(self, completion):
        start = self.cursor_position + getattr(completion, "start_position", 0)
        self.text = self.text[:start] + completion.text + self.text[self.cursor_position:]
        self.cursor_position = start + len(completion.text)
        self.complete_state = None

    def insert_text(self, fragment: str):
        self.text = self.text[:self.cursor_position] + fragment + self.text[self.cursor_position:]
        self.cursor_position += len(fragment)

    def start_completion(self):
        self.started_completion = True


class _FakeSuggestionBuffer:
    def __init__(self, text: str, suggestion_text: str):
        self.text = text
        self.cursor_position = len(text)
        self.complete_state = None
        self.suggestion = SimpleNamespace(text=suggestion_text)
        self.started_completion = False
    ...
    def cursor_right(self, count: int = 1):
...
class TestCompletionAcceptance:
    def test_enter_accepts_completion_without_submitting(self):
        cli = _make_cli()
        buf = _FakeCompletionBuffer("/sh", "shell", start_position=-2)

        handled = cli._accept_selected_completion(buf)

        assert handled is True
        assert buf.text == "/shell "
        assert buf.cursor_position == len("/shell ")

    def test_enter_does_not_add_space_after_colon_completion(self):
        cli = _make_cli()
        buf = _FakeCompletionBuffer("/model anth", "anthropic:", start_position=-4)

        handled = cli._accept_selected_completion(buf)

        assert handled is True
        assert buf.text == "/model anthropic:"
        assert buf.cursor_position == len("/model anthropic:")

    def test_tab_prefers_completion_menu_over_history_suggestion(self):
        cli = _make_cli()
        buf = _FakeSuggestionBuffer("git st", "atus")

        cli._handle_tab_completion(buf)

        assert buf.started_completion is True
        assert buf.text == "git st"

    def test_right_arrow_accepts_history_suggestion(self):
        cli = _make_cli()
        buf = _FakeSuggestionBuffer("git st", "atus")

        cli._handle_right_arrow_suggestion(buf)

        assert buf.text == "git status"
        assert buf.cursor_position == len("git status")

    def test_right_arrow_moves_cursor_when_no_suggestion(self):
        cli = _make_cli()
        buf = _FakeSuggestionBuffer("git status", "")
        buf.suggestion = None
        buf.cursor_position = 2

        cli._handle_right_arrow_suggestion(buf, count=3)
    ...
    def test_history_shows_recent_sessions_when_current_chat_is_empty(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running OWLS",
                "preview": "check running gateways for owls",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running OWLS" in output
...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        ...
        import cli
        monkeypatch.setattr(cli, "_owls_home", owls_home)
    ...
    def test_normalize_root_model_keys_moves_to_model(self):
        ...
        from owls_cli.config import _normalize_root_model_keys
