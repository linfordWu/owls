...
class TestToolProgressScrollback:
    ...
    def test_spinner_still_updates_on_started(self):
        ...
        assert "💻" not in cli._spinner_text
        assert "$" not in cli._spinner_text

    def test_skills_spinner_omits_emoji_on_started(self):
        """skills progress keeps the live spinner text free of the book emoji."""
        cli = _make_cli(tool_progress="all")
        ...
        cli._on_tool_progress(
            ...
            "skills_list",
            "system-monitor-pro",
            {"category": "system-monitor-pro"},
        )

        assert "📚" not in cli._spinner_text
        assert cli._spinner_text == "system-monitor-pro"
