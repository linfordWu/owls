...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("owls/owls-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned owls/* and pr-* branches."""

    def test_prunes_orphaned_owls_branch(self, git_repo):
        """owls/owls-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "owls/owls-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "owls/owls-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "owls/owls-deadbeef" in result.stdout
        ...
        assert "owls/owls-deadbeef" in orphaned
        ...
        assert "owls/owls-deadbeef" not in result.stdout
