...
- Unlimited timeout (OWLS_CRON_TIMEOUT=0)
- Backward compat: OWLS_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """OWLS_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """OWLS_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("OWLS_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """OWLS_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("OWLS_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("OWLS_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_owls_time_importable(self):
        """owls_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _owls_now
        assert callable(_owls_now)

    def test_owls_constants_importable(self):
        """owls_constants should be importable from cron context."""
        from owls_constants import get_owls_home
        assert callable(get_owls_home)
