...
- Path resolution (absolute, relative to OWLS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp OWLS_HOME."""
    owls_home = tmp_path / ".owls"
    owls_home.mkdir()
    (owls_home / "cron").mkdir()
    (owls_home / "cron" / "output").mkdir()
    (owls_home / "scripts").mkdir()
    monkeypatch.setenv("OWLS_HOME", str(owls_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "OWLS_DIR", owls_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", owls_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", owls_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", owls_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("OWLS_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.owls/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("OWLS_SESSION_PLATFORM") is None
        assert os.environ.get("OWLS_SESSION_CHAT_ID") is None
        assert os.environ.get("OWLS_SESSION_CHAT_NAME") is None
