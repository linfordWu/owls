...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "owls"
        assert config.workspace_id == "owls"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.owls/honcho.json
        monkeypatch.setenv("OWLS_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"owls": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="owls"
        ):
            result = config.resolve_session_name("/home/user/owls/subdir")
        assert result == "owls"
...
class TestResolveConfigPath:
    def test_prefers_owls_home_when_exists(self, tmp_path):
        owls_home = tmp_path / "owls"
        owls_home.mkdir()
        local_cfg = owls_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        owls_home = tmp_path / "owls"
        owls_home.mkdir()
        # No honcho.json in OWLS_HOME — also isolate ~/.owls so
    ...
    def test_falls_back_to_global_without_owls_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("OWLS_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_owls(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("OWLS_HONCHO_HOST", None)
            os.environ.pop("OWLS_HOME", None)
            assert resolve_active_host() == "owls"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"OWLS_HONCHO_HOST": "owls.coder"}):
            assert resolve_active_host() == "owls.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OWLS_HONCHO_HOST", None)
            with patch("owls_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "owls.coder"

    def test_default_profile_returns_owls(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OWLS_HONCHO_HOST", None)
            with patch("owls_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "owls"

    def test_custom_profile_returns_owls(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OWLS_HONCHO_HOST", None)
            with patch("owls_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "owls"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OWLS_HONCHO_HOST", None)
            # Temporarily remove owls_cli.profiles to simulate import failure
            saved = sys.modules.get("owls_cli.profiles")
            sys.modules["owls_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "owls"
            finally:
                if saved is not None:
                    sys.modules["owls_cli.profiles"] = saved
                else:
                    sys.modules.pop("owls_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="owls.coder")
        assert config.host == "owls.coder"
        assert config.workspace_id == "owls"  # shared workspace
        assert config.ai_peer == "owls.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="owls")
        assert config.host == "owls"
        assert config.workspace_id == "owls"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "owls": {"aiPeer": "owls", "peerName": "alice"},
                "owls.coder": {
                    "aiPeer": "owls.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="owls.coder", config_path=config_file,
        )
        assert config.host == "owls.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "owls.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="owls.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "owls.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"owls": {"enabled": True, "aiPeer": "owls"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"owls": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_owls_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="owls",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("owls_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_owls_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="owls",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("owls_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
