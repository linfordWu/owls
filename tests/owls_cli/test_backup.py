"""Tests for owls backup and import commands."""
...
def _make_owls_tree(root: Path) -> None:
    """Create a realistic ~/.owls directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "owls_state.db").write_bytes(b"fake-state")
    ...
    # owls repo (should be EXCLUDED)
    (root / "owls").mkdir(exist_ok=True)
    (root / "owls" / "run_agent.py").write_text("# big file\n")
    (root / "owls" / ".git").mkdir()
    (root / "owls" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_owls(self):
        from owls_cli.backup import _should_exclude
        assert _should_exclude(Path("owls/run_agent.py"))
        assert _should_exclude(Path("owls/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        _make_owls_tree(owls_home)

        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        # get_default_owls_root needs this
    ...
    def test_excludes_owls(self, tmp_path, monkeypatch):
        """Backup does NOT include owls/ directory."""
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        _make_owls_tree(owls_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "owls" in n]
            assert agent_files == [], f"owls files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/owls-backup-*.zip."""
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        (owls_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("owls-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid OWLS backup."""
        from owls_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only owls_state.db (old wrong name) is rejected."""
        from owls_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["owls_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into owls home."""
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (owls_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (owls_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (owls_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (owls_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_owls_prefix(self, tmp_path, monkeypatch):
        """Import strips .owls/ prefix if all entries share it."""
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".owls/config.yaml": "model: test\n",
            ".owls/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (owls_home / "config.yaml").read_text() == "model: test\n"
        assert (owls_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_owls_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a owls backup."""
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (owls_home / "config.yaml").exists()
        # traversal file should NOT exist outside owls home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (owls_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (owls_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (owls_home / "config.yaml").read_text() == "model: restored\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".owls"
        src_home.mkdir(parents=True)
        _make_owls_tree(src_home)

        monkeypatch.setenv("OWLS_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from owls_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".owls"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("OWLS_HOME", str(dst_home))
        ...
        # owls should NOT be present
        assert not (dst_home / "owls").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from owls_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without owls markers fails validation."""
        import io
        from owls_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_owls(self):
        """Detects .owls/ prefix wrapping all entries."""
        import io
        from owls_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".owls/config.yaml", "test")
            zf.writestr(".owls/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".owls/"
...
class TestBackupEdgeCases:
    def test_nonexistent_owls_home(self, tmp_path, monkeypatch):
        """Backup exits when owls home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".owls"
        monkeypatch.setenv("OWLS_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("owls-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_owls_home(self, tmp_path, monkeypatch):
        """Backup handles empty owls home (no files to back up)."""
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        # Only excluded dirs, no actual files
        (owls_home / "__pycache__").mkdir()
        (owls_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_skips_output_zip_inside_owls(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside owls root."""
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        (owls_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside owls home
        out_zip = owls_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        (owls_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        (owls_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (owls_home / "config.yaml").exists()
        assert (owls_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (owls_home / "profiles" / "coder" / "config.yaml").exists()
        assert (owls_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "owls -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import owls_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "owls_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from owls_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def owls_home(self, tmp_path):
        """Create a fake OWLS_HOME with critical state files."""
        home = tmp_path / ".owls"
        home.mkdir()
    ...
    def test_creates_snapshot(self, owls_home):
        from owls_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(owls_home=owls_home)
        assert snap_id is not None
        snap_dir = owls_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, owls_home):
        from owls_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", owls_home=owls_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, owls_home):
        from owls_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(owls_home=owls_home)
        db_copy = owls_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, owls_home):
        from owls_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(owls_home=owls_home)
        assert (owls_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, owls_home):
        from owls_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(owls_home=owls_home)
        with open(owls_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(owls_home=empty) is None

    def test_list_snapshots(self, owls_home):
        from owls_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", owls_home=owls_home)
        id2 = create_quick_snapshot(label="second", owls_home=owls_home)

        snaps = list_quick_snapshots(owls_home=owls_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, owls_home):
        from owls_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", owls_home=owls_home)
        snaps = list_quick_snapshots(limit=3, owls_home=owls_home)
        assert len(snaps) == 3

    def test_restore_config(self, owls_home):
        from owls_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(owls_home=owls_home)

        (owls_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (owls_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, owls_home=owls_home)
        assert result is True
        assert "openrouter" in (owls_home / "config.yaml").read_text()

    def test_restore_state_db(self, owls_home):
        from owls_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(owls_home=owls_home)

        conn = sqlite3.connect(str(owls_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, owls_home=owls_home)
    ...
    def test_restore_nonexistent(self, owls_home):
        from owls_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", owls_home=owls_home) is False

    def test_auto_prune(self, owls_home):
        from owls_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", owls_home=owls_home)
        snaps = list_quick_snapshots(limit=100, owls_home=owls_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, owls_home):
        from owls_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", owls_home=owls_home)
        deleted = prune_quick_snapshots(keep=3, owls_home=owls_home)
        assert deleted == 7
        assert len(list_quick_snapshots(owls_home=owls_home)) == 3
