...
Covers:
 - Normal path (tokens already in OWLS auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — OWLS
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `owls auth openai-codex`. The old
...
@pytest.fixture()
def owls_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in OWLS auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(owls_auth_only_env):
    """openai-codex appears when tokens are already in OWLS auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or OWLS
    auth store."""
    owls_home = tmp_path / ".owls"
    owls_home.mkdir()
    ...
    (owls_home / "auth.json").write_text(
