...
class TestGatewayConfigGate:
    ...
    def test_config_gate_excluded_from_help_when_off(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OWLS_HOME", str(tmp_path))
...
class TestSlashCommandCompleter:
    ...
    def test_no_completions_for_non_slash_input_without_explicit_request(self):
        completions = list(
    ...
    def test_linux_command_completion_on_explicit_tab(self, monkeypatch):
        completer = SlashCommandCompleter()
        monkeypatch.setattr(
            completer._shell_completion_engine,
            "_get_shell_command_names",
            lambda: ["git", "grep", "ls"],
        )

        completions = _completions(completer, "gr")

        assert [item.text for item in completions] == ["grep"]

    def test_linux_command_completion_after_shell_wrapper(self, monkeypatch):
        completer = SlashCommandCompleter()
        monkeypatch.setattr(
            completer._shell_completion_engine,
            "_get_shell_command_names",
        ...
        completions = _completions(completer, "sudo gi")

        assert [item.text for item in completions] == ["git"]

    def test_git_subcommand_completion_after_command(self, monkeypatch):
        completer = SlashCommandCompleter()
        monkeypatch.setattr(
            completer._shell_completion_engine,
            "_get_git_subcommand_names",
            lambda: ["branch", "status", "stash"],
        )

        completions = _completions(completer, "git st")

        assert [item.text for item in completions] == ["stash", "status"]

    def test_git_subcommand_completion_after_command_with_trailing_space(self, monkeypatch):
        completer = SlashCommandCompleter()
        monkeypatch.setattr(
            completer._shell_completion_engine,
            "_get_git_subcommand_names",
        ...
        completions = _completions(completer, "git ")

        assert [item.text for item in completions] == ["branch", "stash", "status"]

    def test_git_option_completion_after_subcommand(self):
        completions = _completions(SlashCommandCompleter(), "git status --")
    ...
    def test_git_path_completion_after_file_subcommand(self, tmp_path, monkeypatch):
        (tmp_path / "README.md").write_text("readme")
        (tmp_path / "src").mkdir()
        monkeypatch.chdir(tmp_path)

        completions = _completions(SlashCommandCompleter(), "git add RE")

        assert [item.text for item in completions] == ["README.md"]

    def test_cd_completion_suggests_directories_without_path_prefix(self, tmp_path, monkeypatch):
        (tmp_path / "src").mkdir()
        (tmp_path / "scripts").mkdir()
        (tmp_path / "setup.py").write_text("print('ok')")
        monkeypatch.chdir(tmp_path)

        completions = _completions(SlashCommandCompleter(), "cd s")

        texts = {item.text for item in completions}

        assert "scripts/" in texts
        assert "src/" in texts
        assert "setup.py" not in texts

    def test_ls_completion_suggests_options_and_paths(self, tmp_path, monkeypatch):
        (tmp_path / "src").mkdir()
        (tmp_path / "README.md").write_text("readme")
        monkeypatch.chdir(tmp_path)

        completions = _completions(SlashCommandCompleter(), "ls ")
    ...
    def test_rm_completion_suggests_files_in_argument_position(self, tmp_path, monkeypatch):
        (tmp_path / "README.md").write_text("readme")
        (tmp_path / "notes.txt").write_text("notes")
        monkeypatch.chdir(tmp_path)

        completions = _completions(SlashCommandCompleter(), "rm RE")

        assert [item.text for item in completions] == ["README.md"]

    def test_systemctl_subcommand_completion_after_command(self):
        completer = SlashCommandCompleter()
        completions = _completions(completer, "systemctl re")
    ...
    def test_export_completion_suggests_environment_variables(self, monkeypatch):
        monkeypatch.setenv("PATH", "/usr/bin")
        monkeypatch.setenv("PYTHONPATH", "/tmp/python")

        completions = _completions(SlashCommandCompleter(), "export PY")

        texts = {item.text for item in completions}

        assert "PYTHONPATH=" in texts

    def test_man_completion_suggests_command_names(self, monkeypatch):
        completer = SlashCommandCompleter()
        monkeypatch.setattr(
            completer._shell_completion_engine,
            "_get_shell_command_names",
            lambda: ["python", "pytest", "sed"],
        )

        completions = _completions(completer, "man py")
    ...
    def test_tmux_completion_suggests_subcommands(self):
        completions = _completions(SlashCommandCompleter(), "tmux ne")

        texts = {item.text for item in completions}

        assert "new-session" in texts
        assert "new-window" in texts

    def test_vim_completion_suggests_files(self, tmp_path, monkeypatch):
        (tmp_path / "README.md").write_text("readme")
        (tmp_path / "docs.md").write_text("docs")
        monkeypatch.chdir(tmp_path)

        completions = _completions(SlashCommandCompleter(), "vim RE")
...
from owls_cli.commands import discord_skill_commands_by_category  # noqa: E402
