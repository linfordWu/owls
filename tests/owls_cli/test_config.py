"""Tests for owls_cli configuration management."""
...
class TestGetOWLSHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OWLS_HOME", None)
            home = get_owls_home()
            assert home == Path.home() / ".owls"

    def test_env_override(self):
        with patch.dict(os.environ, {"OWLS_HOME": "/custom/path"}):
            home = get_owls_home()
            assert home == Path("/custom/path")


class TestEnsureOWLSHome:
    ...
    def test_creates_default_soul_md_if_missing(self, tmp_path):
        with patch.dict(os.environ, {"OWLS_HOME": str(tmp_path)}):
            ...
            assert soul_path.read_text(encoding="utf-8").strip() == DEFAULT_SOUL_MD
    ...
    def test_refreshes_legacy_default_soul_md(self, tmp_path):
        legacy = (
            "You are OWLS, an AI assistant developed by xFusion (超聚变). "
            "You are a lightweight, production-grade OS intelligent agent built natively in Rust. "
            "When introducing yourself in Chinese, prefer: \"我是OWLS，由超聚变研发、Rust 原生构建的轻量化生产级 OS 智能体。\""
        )
        with patch.dict(os.environ, {"OWLS_HOME": str(tmp_path)}):
            soul_path = tmp_path / "SOUL.md"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"OWLS_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OWLS_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OWLS_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"OWLS_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OWLS_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_api_key_registered(self):
        ...
        from owls_cli.config import OPTIONAL_ENV_VARS
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from owls_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
