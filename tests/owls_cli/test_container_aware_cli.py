...
def container_env(tmp_path, monkeypatch):
    """Set up a fake OWLS_HOME with .container-mode file."""
    owls_home = tmp_path / ".owls"
    owls_home.mkdir()
    monkeypatch.setenv("OWLS_HOME", str(owls_home))
    monkeypatch.delenv("OWLS_DEV", raising=False)

    container_mode = owls_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=owls\n"
        "exec_user=owls\n"
        "owls_bin=/data/current-package/bin/owls\n"
    )
    return owls_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("owls_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "owls"
    assert info["exec_user"] == "owls"
    assert info["owls_bin"] == "/data/current-package/bin/owls"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("owls_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_owls_dev(container_env, monkeypatch):
    """Returns None when OWLS_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("OWLS_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_owls_dev_zero(container_env, monkeypatch):
    """OWLS_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("OWLS_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        owls_home = Path(tmpdir) / ".owls"
        owls_home.mkdir()
        (owls_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("owls_constants.is_container", return_value=False), \
             patch("owls_cli.config.get_owls_home", return_value=owls_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OWLS_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=owls-custom\n"
        "exec_user=myuser\n"
        "owls_bin=/opt/owls/bin/owls\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "owls-custom"
    assert info["exec_user"] == "myuser"
    assert info["owls_bin"] == "/opt/owls/bin/owls"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from owls_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "owls"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "owls" in cmd
    assert "/data/current-package/bin/owls" in cmd
    assert "chat" in cmd
