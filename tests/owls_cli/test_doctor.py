...
import owls_cli.doctor as doctor
import owls_cli.gateway as gateway_cli
from owls_cli import doctor as doctor_mod
from owls_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "OWLS_HOME", owls_home)
    monkeypatch.delenv("OWLS_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("OWLS_INTERACTIVE")
        raise SystemExit(0)
...
def test_check_gateway_service_linger_warns_when_disabled(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "owls-gateway.service"
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_owls_home(self, tmp_path, provider=""):
        """Create a minimal OWLS_HOME with config.yaml."""
        home = tmp_path / ".owls"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_owls_home(tmp_path, provider)
