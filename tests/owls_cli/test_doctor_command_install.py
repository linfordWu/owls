"""Tests for the Command Installation check in owls doctor."""
...
import owls_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal OWLS_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".owls"
    ...
    owls_bin = venv_bin_dir / "owls"
    owls_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    owls_bin.chmod(0o755)

    monkeypatch.setattr(doctor_mod, "OWLS_HOME", home)
    ...
    # Stub auth checks
    try:
        from owls_cli import auth as _auth_mod
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, owls_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "owls"
        cmd_link.symlink_to(owls_bin)
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "owls"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == owls_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "owls"
        wrong_target = tmp_path / "wrong_owls"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        owls_bin = project / ".venv" / "bin" / "owls"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "owls"
        cmd_link.symlink_to(owls_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/owls" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/owls is a regular file (not symlink), accept it."""
        home, project, owls_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "owls"
        cmd_link.write_text("#!/bin/sh\nexec python -m owls_cli.main \"$@\"\n")
