"""Tests for the top-level `./owls` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./owls` should use `owls_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "owls"
    called = []

    fake_main_module = types.ModuleType("owls_cli.main")

    def fake_main():
        called.append("owls_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "owls_cli.main", fake_main_module)
