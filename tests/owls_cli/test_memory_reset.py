"""Tests for the `owls memory reset` CLI command.
...
- Profile-scoped reset (uses OWLS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake OWLS_HOME with memory files."""
    owls_home = tmp_path / ".owls"
    memories = owls_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("OWLS_HOME", str(owls_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nOWLS repo is at ~/.owls/owls\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `owls memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        owls_home = tmp_path / ".owls"
        (owls_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's OWLS_HOME."""
        ...
        monkeypatch.setenv("OWLS_HOME", str(profile_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        owls_home = tmp_path / ".owls"
        owls_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("OWLS_HOME", str(owls_home))

        # The memories dir won't exist; get_owls_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
