"""Tests for the update check mechanism in owls_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("owls_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OWLS_HOME", str(tmp_path))
    with patch("owls_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "owls_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when OWLS_HOME has no git repo."""
    import owls_cli.banner as banner
    ...
    # Point OWLS_HOME at a temp dir with no owls/.git
    monkeypatch.setenv("OWLS_HOME", str(tmp_path))
    with patch("owls_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from owls_cli.main import _invalidate_update_cache

    # Build a fake ~/.owls with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"OWLS_HOME": str(default_home)}):
