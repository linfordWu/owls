...
Ensures ``owls update`` correctly detects running gateways managed by
...
import owls_cli.gateway as gateway_cli
import owls_cli.main as cli_main
...
def _no_restart_verify_sleep(monkeypatch):
    """owls_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units owls-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="owls-gateway.service loaded active running OWLS Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>OWLS_HOME</key>" in plist
...
class TestLaunchdPlistCurrentness:
    def test_launchd_plist_is_current_ignores_path_drift(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.owls.gateway.plist"
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: owls gateway run'."""
        ...
        assert "Restart manually: owls gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: owls gateway run" in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted owls-gateway" in captured
...
class TestServicePidExclusion:
    ...
    where ``owls update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.owls.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="owls-gateway.service loaded inactive dead OWLS Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
...
class TestFindGatewayPidsExclude:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        profile_dir = tmp_path / ".owls" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(gateway_cli, "is_windows", lambda: False)
        monkeypatch.setattr(gateway_cli, "get_owls_home", lambda: profile_dir)

        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.owls/owls/venv/bin/python -m owls_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.owls/owls/venv/bin/python -m owls_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda owls_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``owls update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point OWLS_HOME at a temp dir so the marker file lands there
        owls_home = tmp_path / ".owls"
        owls_home.mkdir()
        monkeypatch.setenv("OWLS_HOME", str(owls_home))
        import owls_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_owls_home", lambda: owls_home)
        # Also patch the module-level ref used by cmd_update
        import owls_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_owls_home", lambda: owls_home)
        ...
        exit_code_path = owls_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy owls.service warning printed by `owls update`.

    Users who installed OWLS before the service rename often have a
    dormant ``owls.service`` that starts flap-fighting the current
    ``owls-gateway.service`` after PR #5646. Every ``owls update``
    should remind them to run ``owls gateway migrate-legacy`` until
    they do.
    """

    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=OWLS Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m owls_cli.main gateway run --replace\n"
    )
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "owls.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy OWLS gateway unit(s) detected" in captured
        assert "owls.service" in captured
        assert "owls gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy OWLS gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (owls-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``owls.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "owls-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "owls-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy OWLS gateway" not in captured
        assert "owls-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_skips_legacy_check_on_non_systemd_platforms(
        ...
        (user_dir / "owls.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        (system_dir / "owls.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        ...
        captured = capsys.readouterr().out
        assert "Legacy OWLS gateway" in captured
