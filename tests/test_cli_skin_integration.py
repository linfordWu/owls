...
from cli import OWLSCLI, _build_compact_banner, _rich_text_from_ansi
from owls_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("owls_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_uses_minimal_shell_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
               patch("cli.format_banner_version_label", return_value="v1.0"):
            banner = _build_compact_banner()

        assert "OWLS Shell" in banner
        assert "NOUS OWLS" not in banner
    ...
    def test_poseidon_compact_banner_uses_skin_branding(self):
        ...
        assert "Poseidon Agent Shell" in banner
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        assert "OWLS Shell (v1.0)" in banner
        assert "Workdir |" in banner
