"""
Tests for mcp_serve — OWLS MCP server.
...
def _isolate_owls_home(tmp_path, monkeypatch):
    """Redirect OWLS_HOME to a temp directory."""
    monkeypatch.setenv("OWLS_HOME", str(tmp_path))
    try:
        import owls_constants
        monkeypatch.setattr(owls_constants, "get_owls_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking owls_state schema."""
