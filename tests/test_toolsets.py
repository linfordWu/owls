...
class TestToolsetConsistency:
    ...
    def test_owls_platforms_share_core_tools(self):
        """All owls-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord_server`` on
        owls-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["owls-cli", "owls-telegram", "owls-discord", "owls-whatsapp", "owls-slack", "owls-signal", "owls-homeassistant"]
