...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    assert server.os.environ["OWLS_GATEWAY_SESSION"] == "1"
    assert server.os.environ["OWLS_EXEC_ASK"] == "1"
    assert server.os.environ["OWLS_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("owls_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_reasoning_updates_live_session_and_agent(tmp_path, monkeypatch):
    monkeypatch.setattr(server, "_owls_home", tmp_path)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("owls_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("owls_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("owls_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch("owls_cli.commands.SlashCommandCompleter", side_effect=Exception("no completer")):
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `owls model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `owls model` and the classic CLI /model picker.
    ...
    with patch(
        "owls_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("owls_cli.models.provider_model_ids") as live_fetch:
