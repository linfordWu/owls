...
Validates that:
1. The installed atroposlib API still matches OWLS's expectations
2. OWLS's parser registry remains compatible with ManagedServer parsing
3. OWLSBaseEnv wires the selected parser into ServerManager correctly

These tests verify the contract between owls's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what owls expects."""
...
class TestParserCompatibility:
    """Test that owls's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    """Test that owls_base_env.py's tool-parser wiring matches the current API."""

    def test_owls_base_env_sets_server_manager_tool_parser(self):
        """OWLS wires parser selection through ServerManager.tool_parser."""
        import ast

        base_env_path = Path(__file__).parent.parent.parent / "environments" / "owls_base_env.py"
        ...
        assert found_assignment, (
            "owls_base_env.py should set self.server.tool_parser from config.tool_call_parser"
        )

    def test_owls_base_env_uses_config_tool_call_parser(self):
        """Verify owls_base_env uses the config field rather than a local parser instance."""
        base_env_path = Path(__file__).parent.parent.parent / "environments" / "owls_base_env.py"
        source = base_env_path.read_text()
