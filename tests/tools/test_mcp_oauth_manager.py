...
def test_owls_provider_subclass_exists():
    """OWLSMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _OWLS_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _OWLS_PROVIDER_CLS is not None
    assert issubclass(_OWLS_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_owls_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns OWLSMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _OWLS_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("OWLS_HOME", str(tmp_path))
    ...
    assert _OWLS_PROVIDER_CLS is not None
    assert isinstance(provider, _OWLS_PROVIDER_CLS)
    assert provider._owls_server_name == "srv"
