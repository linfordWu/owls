...
def _restore_tool_modules():
    original_owls_home = os.environ.get("OWLS_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_owls_home is None:
            os.environ.pop("OWLS_HOME", None)
        else:
            os.environ["OWLS_HOME"] = original_owls_home
        _reset_modules(("tools", "owls_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    owls_cli = types.ModuleType("owls_cli")
    owls_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["owls_cli"] = owls_cli
    owls_home = tmp_path / "owls-home"
    os.environ["OWLS_HOME"] = str(owls_home)
    sys.modules["owls_cli.config"] = types.SimpleNamespace(
        get_owls_home=lambda: owls_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="owls", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": owls_home / "modal_snapshots.json",
