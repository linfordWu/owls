...
class TestSyncSkills:
    ...
    def test_unmodified_skill_moved_to_new_category(self, tmp_path):
        """A bundled skill moved between categories should migrate if unmodified."""
        bundled = tmp_path / "bundled_skills"
        new_bundled_skill = bundled / "linux" / "old-skill"
        new_bundled_skill.mkdir(parents=True)
        (new_bundled_skill / "SKILL.md").write_text(
            "---\nname: old-skill\n---\n# Old v2"
        )

        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        old_user_skill = skills_dir / "old-skill"
        old_user_skill.mkdir(parents=True)
        (old_user_skill / "SKILL.md").write_text("---\nname: old-skill\n---\n# Old v1")
        old_origin_hash = _dir_hash(old_user_skill)
        ...
        assert not old_user_skill.exists()
        assert (skills_dir / "linux" / "old-skill" / "SKILL.md").read_text() == (
            "---\nname: old-skill\n---\n# Old v2"
        )

    def test_removed_bundled_skill_deletes_unmodified_user_copy(self, tmp_path):
        """Removed bundled skills should remove unmodified synced copies."""
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"

        removed_skill = skills_dir / "removed-skill"
        removed_skill.mkdir(parents=True)
        (removed_skill / "SKILL.md").write_text(
            "---\nname: removed-skill\n---\n# Removed"
        )
        removed_hash = _dir_hash(removed_skill)
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """OWLS_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("OWLS_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("OWLS_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty OWLS_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("OWLS_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
