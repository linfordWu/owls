...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._owls_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_owls_bin(self):
        """After _INSTALL_FAILED, manual install in OWLS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        owls_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(owls_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(owls_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._owls_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == owls_bin
            assert _tirith_mod._resolved_path == owls_bin
            mock_clear.assert_called_once()
...
class TestOWLSHomeIsolation:
    def test_owls_bin_dir_respects_owls_home(self):
        """_owls_bin_dir must use OWLS_HOME, not hardcoded ~/.owls."""
        from tools.tirith_security import _owls_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"OWLS_HOME": tmpdir}):
            result = _owls_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_owls_home(self):
        """_failure_marker_path must use OWLS_HOME, not hardcoded ~/.owls."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"OWLS_HOME": "/custom/owls"}):
            result = _failure_marker_path()
        assert result == "/custom/owls/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets OWLS_HOME; verify it's active."""
        owls_home = os.getenv("OWLS_HOME")
        assert owls_home is not None, "OWLS_HOME should be set by conftest"
        assert "owls_test" in owls_home, "Should point to test temp dir"

    def test_get_owls_home_fallback(self):
        """Without OWLS_HOME set, falls back to ~/.owls."""
        from tools.tirith_security import _get_owls_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove OWLS_HOME entirely
            os.environ.pop("OWLS_HOME", None)
            result = _get_owls_home()
        assert result == os.path.join(os.path.expanduser("~"), ".owls")
