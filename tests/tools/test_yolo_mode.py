"""Tests for --yolo (OWLS_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When OWLS_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("OWLS_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With OWLS_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """OWLS_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("OWLS_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for OWLS_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("OWLS_YOLO_MODE", "")
        monkeypatch.setenv("OWLS_INTERACTIVE", "1")
        monkeypatch.setenv("OWLS_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("OWLS_YOLO_MODE") returns ""
