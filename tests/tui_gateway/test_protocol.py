...
def server():
    with patch.dict("sys.modules", {
        "owls_constants": MagicMock(get_owls_home=MagicMock(return_value="/tmp/owls_test")),
        "owls_cli.env_loader": MagicMock(),
        "owls_cli.banner": MagicMock(),
        "owls_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/owls-dev": {"name": "owls-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "owls-dev", "session_id": sid},
        })
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "owls-dev", "session_id": sid},
        })
