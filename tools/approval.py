...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("OWLS_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $OWLS_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_OWLS_ENV_PATH = (
    r'(?:~\/\.owls/|'
    r'(?:\$home|\$\{home\})/\.owls/|'
    r'(?:\$owls_home|\$\{owls_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bowls\s+gateway\s+(stop|restart)\b', "stop/restart owls gateway (kills running agents)"),
    (r'\bowls\s+update\b', "owls update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart owls-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart owls-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(owls|gateway|cli\.py)\b', "kill owls/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill owls` but not
    # `kill -9 $(pgrep -f owls)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("OWLS_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("OWLS_INTERACTIVE")
    is_gateway = os.getenv("OWLS_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("OWLS_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("OWLS_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("OWLS_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("OWLS_INTERACTIVE")
    is_gateway = os.getenv("OWLS_GATEWAY_SESSION")
    is_ask = os.getenv("OWLS_EXEC_ASK")
