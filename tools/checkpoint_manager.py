...
Architecture:
    ~/.owls/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        OWLS_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_owls_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("OWLS_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal OWLS infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "owls@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "OWLS Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "OWLS_WORKDIR").write_text(
...
class CheckpointManager:
            ...
            if ok:
                try:
                    ...
                    get_audit_logger().log_event({
                        ...
                        "command": f"checkpoint for {abs_dir}",
                        "description": f"Checkpoint created: {reason}",
                        "risk_level": "low",
                    })
                except Exception:
                    pass
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None) -> Dict:
        ...
        # For full-directory restore, also remove files created after the
        # checkpoint.  ``git checkout <hash> -- .`` only updates tracked files;
        # it does not delete new untracked files.  We reset the index to the
        # checkpoint tree, force-update the working tree, then clean untracked
        # files so the directory exactly matches the checkpoint state.
        if not file_path:
            _run_git(["read-tree", commit_hash], shadow, abs_dir)
            _run_git(["checkout-index", "-a", "-f"], shadow, abs_dir)
            _run_git(["clean", "-fd"], shadow, abs_dir)
        ...
        # Audit: log checkpoint restore
        try:
            from tools.audit_logger import get_audit_logger
            get_audit_logger().log_event({
                "event_type": "checkpoint_restore",
                "checkpoint_hash": commit_hash,
                "command": f"restore {abs_dir}",
                "description": f"Restored to {commit_hash[:8]}: {reason}",
                "risk_level": "medium",
            })
        except Exception:
            pass
