...
def _get_safe_write_root() -> Optional[str]:
    """Return the resolved OWLS_WRITE_SAFE_ROOT path, or None if unset.
    ...
    """
    root = os.getenv("OWLS_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
@dataclass
class BackupResult:
    """Result from creating a file backup."""
    success: bool = False
...
# ---------------------------------------------------------------------------
# File backup safety helpers
# ---------------------------------------------------------------------------

_BACKUP_BASE_DIR = get_owls_home() / "backups"


def _backup_dir_for_today() -> Path:
    """Return the backup directory for today."""
    today = datetime.now().strftime("%Y-%m-%d")
    return _BACKUP_BASE_DIR / today
...
def _print_backup_status(result: BackupResult) -> None:
    """Print backup status to terminal for user visibility.

    The todolist requires clear terminal output of backup path, time, and status.
    """
    ts = result.timestamp or datetime.now().strftime("%H:%M:%S")
    if result.success:
        if result.size_bytes >= 1024:
            size_str = f"{result.size_bytes / 1024:.1f} KB"
        else:
            size_str = f"{result.size_bytes} B"
        print(
            f"[BACKUP] {ts} | {result.original_path} -> {result.backup_path} "
            f"({size_str}) | 状态: 成功"
        )
    else:
        print(
            f"[BACKUP] {ts} | {result.original_path} | 状态: 失败 | 错误: {result.error}"
        )


def create_file_backup(path: str) -> BackupResult:
    """Create a backup of a file before modification.

    Backups are stored in ~/.owls/backups/YYYY-MM-DD/ with a timestamp prefix.
    Returns BackupResult indicating success/failure.
    """
    import os as _os
    ...
    if not _os.path.exists(expanded):
        ...
        return BackupResult(success=True, original_path=path)

    if _os.path.isdir(expanded):
        return BackupResult(
            success=False,
            original_path=path,
            error="Cannot backup directories (only files)",
        )

    try:
        backup_dir = _backup_dir_for_today()
        ...
        while backup_path.exists():
            backup_name = f"{timestamp}_{counter}_{basename}"
            backup_path = backup_dir / backup_name
            counter += 1

        shutil.copy2(expanded, backup_path)
        size_bytes = _os.path.getsize(backup_path)
...
class ShellFileOperations(FileOperations):
    ...
    def __init__(self, terminal_env, cwd: str = None):
        ...
        # File backup safety: when True, all modifying operations create a backup first.
        # Controlled by OWLS_FILE_BACKUP_REQUIRED env var (default: true for safety).
        _backup_env = os.getenv("OWLS_FILE_BACKUP_REQUIRED", "true").lower()
        self.backup_required = _backup_env in ("true", "1", "yes", "on")
        self._last_backup: Optional[Dict[str, Any]] = None
    ...
    def _backup_before_modify(self, path: str) -> Optional[str]:
        """Create a backup of *path* before modifying it.

        Returns None on success (or when the file doesn't exist yet),
        or an error string when backup is required but fails.
        Stores the backup result dict in self._last_backup for callers
        to include in their return values.
        """
        ...
        # Check if file exists via the terminal environment (works for local and remote).
        exists_cmd = f"test -f {self._escape_shell_arg(expanded)} && echo 'yes' || echo 'no'"
        exists_result = self._exec(exists_cmd)
        file_exists = exists_result.stdout.strip() == 'yes'
        if not file_exists:
            return None  # New file — nothing to backup

        # For local environments, use the direct filesystem backup.
        # For remote/container environments, copy via shell to the backup dir.
        backup_dir = _backup_dir_for_today()
        backup_dir.mkdir(parents=True, exist_ok=True)
        timestamp = _format_backup_timestamp()
        ...
        # Check if the expanded path is accessible on the local filesystem.
        if os.path.exists(expanded) and not os.path.isdir(expanded):
            # Local file — direct copy
            try:
                shutil.copy2(expanded, local_path)
                size_bytes = os.path.getsize(local_path)
                result = BackupResult(
                    success=True, backup_path=local_path,
                    original_path=path, timestamp=timestamp, size_bytes=size_bytes,
                )
                self._last_backup = result.to_dict()
                _print_backup_status(result)
                return None
            except Exception as e:
                result = BackupResult(
                    success=False, original_path=path, error=str(e),
                )
                self._last_backup = result.to_dict()
                _print_backup_status(result)
                return f"Backup failed: {e}. File modification blocked for safety."
        else:
            # Remote environment — try to copy via shell to a temp location,
            # then pull it back to the local backup dir.
            remote_tmp = f"/tmp/owls_backup_{timestamp}_{basename}"
            cp_cmd = f"cp {self._escape_shell_arg(expanded)} {self._escape_shell_arg(remote_tmp)} 2>/dev/null && echo 'ok' || echo 'fail'"
            cp_result = self._exec(cp_cmd)
            if cp_result.stdout.strip() == 'ok':
                # Try to read the remote temp file content and write locally.
                cat_cmd = f"cat {self._escape_shell_arg(remote_tmp)}"
                cat_result = self._exec(cat_cmd)
                if cat_result.exit_code == 0:
                    try:
                        with open(local_path, 'wb') as f:
                            f.write(cat_result.stdout.encode('utf-8', errors='replace'))
                        size_bytes = os.path.getsize(local_path)
                        result = BackupResult(
                            success=True, backup_path=local_path,
                            original_path=path, timestamp=timestamp, size_bytes=size_bytes,
                        ...
                        self._exec(f"rm -f {self._escape_shell_arg(remote_tmp)}")
                        return None
                    except Exception as e:
                        pass
                self._exec(f"rm -f {self._escape_shell_arg(remote_tmp)}")

            # Remote backup failed — log but do NOT block (remote envs may
            # have checkpoint manager as fallback).
            result = BackupResult(
                success=False, original_path=path,
                error="Remote backup unavailable (file on remote host/container)",
            )
            self._last_backup = result.to_dict()
            _print_backup_status(result)
            return None
    ...
    def delete_file(self, path: str) -> WriteResult:
        ...
        # Safety: backup existing file before deleting
        backup_err = self._backup_before_modify(path)
        if backup_err:
        ...
        wr = WriteResult(backup=self._last_backup)
        self._last_backup = None
        return wr
    ...
    def move_file(self, src: str, dst: str) -> WriteResult:
        ...
        # Safety: backup destination file before overwriting
        backup_err = self._backup_before_modify(dst)
        if backup_err:
            return WriteResult(error=backup_err)
        result = self._exec(
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        # Capture backup from write_result before it's cleared
        _backup = write_result.backup
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        # Wrap write/delete/move so backups are captured in the result.
        all_backups: List[Dict[str, Any]] = []
        orig_write = self.write_file
        orig_delete = self.delete_file
        orig_move = self.move_file

        def _capture_backup(method):
            def wrapper(*args, **kwargs):
                result = method(*args, **kwargs)
                if getattr(result, 'backup', None):
                    all_backups.append(result.backup)
                return result
            return wrapper

        self.write_file = _capture_backup(orig_write)
        self.delete_file = _capture_backup(orig_delete)
        self.move_file = _capture_backup(orig_move)

        try:
            result = apply_v4a_operations(operations, self)
        finally:
            self.write_file = orig_write
            self.delete_file = orig_delete
            self.move_file = orig_move

        if all_backups:
            result.backup = all_backups[0] if len(all_backups) == 1 else all_backups
        return result
