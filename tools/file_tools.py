...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── OWLS internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from owls_constants import get_owls_home as _get_hh
        _owls_home = _get_hh().resolve()
        _blocked_dirs = [
            _owls_home / "skills" / ".hub" / "index-cache",
            _owls_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal OWLS cache file "
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    try:
        ...
        # Hint when old_string not found in replace mode — saves iterations
        # where the agent retries with stale content instead of re-reading.
        if mode == "replace" and result_dict.get("error") and "Could not find" in str(result_dict["error"]):
