...
This module provides the glue:
    - ``OWLSTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "OWLS"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses OWLS_HOME so each profile gets its own OAuth tokens.
    Layout: ``OWLS_HOME/mcp-tokens/``
    """
    try:
        from owls_constants import get_owls_home
        base = Path(get_owls_home())
    except ImportError:
        base = Path(os.environ.get("OWLS_HOME", str(Path.home() / ".owls")))
...
# ---------------------------------------------------------------------------
# OWLSTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class OWLSTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        OWLS_HOME/mcp-tokens/<server_name>.json         -- tokens
        OWLS_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # OWLS records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to OWLS.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "OWLS")
    scope = cfg.get("scope")
