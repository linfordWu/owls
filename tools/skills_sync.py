...
Copies bundled skills from the repo's skills/ directory into ~/.owls/skills/
...
Update logic:
  ...
  - MOVED bundled skills (same name, new category path): unmodified user copy is
    migrated to the new bundled path.
  - DELETED by user (in manifest, absent from user dir): respected, not re-added.
  - REMOVED from bundled (in manifest, gone from repo): cleaned from manifest;
    unmodified user copies are removed.

The manifest lives at ~/.owls/skills/.bundled_manifest.
"""
...
def _get_bundled_dir() -> Path:
    ...
    Checks OWLS_BUNDLED_SKILLS env var first (set by Nix wrapper),
    then falls back to the relative path from this source file.
    """
    env_override = os.getenv("OWLS_BUNDLED_SKILLS")
    if env_override:
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.owls/skills/mlops/axolotl
    """
...
def _find_installed_skill_dir(skill_name: str, expected_hash: str = "") -> Path | None:
    """Find an installed skill by frontmatter name, optionally requiring a hash match."""
    ...
    for skill_md in SKILLS_DIR.rglob("SKILL.md"):
        ...
        if "/.git/" in path_str or "/.github/" in path_str or "/.hub/" in path_str:
            continue
        skill_dir = skill_md.parent
        installed_name = _read_skill_name(skill_md, skill_dir.name)
        if installed_name != skill_name:
            continue
        if expected_hash and _dir_hash(skill_dir) != expected_hash:
            continue
        return skill_dir
    return None
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.owls/skills/ using the manifest.
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        if skill_name not in manifest:
            ...
            # ── In manifest but not at the current destination ──
            # If the bundled skill was moved between categories, migrate the
            # user's unmodified copy to the new path. If no unmodified copy is
            # found, preserve the historical "user deleted it" behavior.
            origin_hash = manifest.get(skill_name, "")
            moved_from = (
                _find_installed_skill_dir(skill_name, origin_hash)
                if origin_hash
                else None
            )
            if moved_from and moved_from != dest:
                try:
                    dest.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copytree(skill_src, dest)
                    shutil.rmtree(moved_from, ignore_errors=True)
                    manifest[skill_name] = bundled_hash
                    updated.append(skill_name)
                    if not quiet:
                        print(f"  ↪ {skill_name} (moved)")
                except (OSError, IOError) as e:
                    if not quiet:
    ...
    for name in cleaned:
        ...
        installed_dir = (
            _find_installed_skill_dir(name, origin_hash)
            if origin_hash
            else None
        )
        if installed_dir:
            try:
                ...
                logger.debug("Could not remove stale bundled skill %s: %s", installed_dir, e)
        del manifest[name]
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `owls skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `owls update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.owls/skills/ ...")
