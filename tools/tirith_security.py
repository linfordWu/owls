...
it is automatically downloaded from GitHub releases to $OWLS_HOME/bin/tirith.
...
def _get_owls_home() -> str:
    """Return the OWLS home directory, respecting OWLS_HOME env var."""
    return str(get_owls_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_owls_home(), ".tirith-install-failed")
...
def _owls_bin_dir() -> str:
    """Return $OWLS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_owls_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $OWLS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_owls_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $OWLS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $OWLS_HOME/bin/tirith
    ...
    owls_bin = os.path.join(_owls_bin_dir(), "tirith")
    if os.path.isfile(owls_bin) and os.access(owls_bin, os.X_OK):
        _resolved_path = owls_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return owls_bin
