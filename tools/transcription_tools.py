...
LOCAL_STT_COMMAND_ENV = "OWLS_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "OWLS_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set OWLS_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="owls-local-stt-") as output_dir:
