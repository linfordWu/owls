...
Configuration is loaded from ~/.owls/config.yaml under the 'tts:' key.
...
from tools.xai_http import owls_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_owls_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.owls/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.owls/config.yaml.
    ...
    try:
        ...
        logger.debug("owls_cli.config not available, using default TTS config")
        return {}
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when OWLS actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": owls_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.owls/config.yaml (tts: section).
    ...
    platform = get_session_env("OWLS_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run owls setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_owls_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
