...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("OWLS_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
async def vision_analyze_tool(
    ...
    try:
        ...
        # OCR pre-processing for text-dense images (screenshots, scans)
        ocr_text = ""
        try:
            from tools.ocr_tool import _is_text_dense_image, ocr_extract_tool
            if _is_text_dense_image(str(temp_image_path)):
                logger.info("Text-dense image detected — running OCR pre-processing")
                ocr_result = json.loads(ocr_extract_tool(str(temp_image_path)))
                ocr_text = ocr_result.get("text", "")
                if ocr_text:
                    logger.info("OCR extracted %d characters", len(ocr_text))
        except Exception as e:
            logger.debug("OCR pre-processing skipped: %s", e)
        ...
        if ocr_text:
            comprehensive_prompt = (
                f"[OCR pre-extracted text from image]:\n{ocr_text}\n\n"
                f"{comprehensive_prompt}"
            )
