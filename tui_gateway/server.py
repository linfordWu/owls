...
load_owls_dotenv(owls_home=_owls_home, project_env=Path(__file__).parent.parent / ".env")
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, float(os.environ.get("OWLS_TUI_SLASH_TIMEOUT_S", "45") or 45))
...
_pool = concurrent.futures.ThreadPoolExecutor(
    max_workers=max(2, int(os.environ.get("OWLS_TUI_RPC_POOL_WORKERS", "4") or 4)),
...
class _SlashWorker:
    """Persistent OWLSCLI subprocess for slash commands."""
...
def resolve_skin() -> dict:
    try:
        from owls_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from owls_cli.model_switch import parse_model_flags, switch_model
    ...
    os.environ["OWLS_MODEL"] = result.new_model
    if persist_global:
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from owls_cli.banner import get_available_skills
    ...
    try:
        ...
        info["mcp_servers"] = []
    try:
        from owls_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from owls_cli.config import save_env_value_secure
...
def _resolve_personality_prompt(cfg: dict) -> str:
    ...
    try:
        ...
        try:
            from owls_cli.config import load_config as _load_full_cfg
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(f"owls_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json")
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from datetime import datetime
        from owls_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _owls_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from owls_cli.models import list_available_providers, normalize_provider
            model = _resolve_model()
            parts = model.split("/", 1)
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_owls_home), "display": display_owls_home()})
    if key == "full":
...
def _(rid, params: dict) -> dict:
    try:
        from owls_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from owls_cli.commands import COMMAND_REGISTRY, SUBCOMMANDS, _build_description
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `owls` is interactive — use `/owls chat -q …` or run `owls` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`owls setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`owls gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`owls sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`owls config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m owls_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "owls_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from owls_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `owls model` and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            from owls_cli.voice import start_recording
            start_recording()
            return _ok(rid, {"status": "recording"})
        if action == "stop":
            from owls_cli.voice import stop_and_transcribe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from owls_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("OWLS_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("OWLS_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [{
            ...
            "rows": [
                ...
                ["Config File", str(_owls_home / "config.yaml")],
            ]
        }]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from owls_cli.tools_config import (
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "search":
            from owls_cli.skills_hub import unified_search, GitHubAuth, create_source_router
        ...
        if action == "install":
            from owls_cli.skills_hub import do_install
            class _Q:
                def print(self, *a, **k): pass
            do_install(query, skip_confirm=True, console=_Q())
            return _ok(rid, {"installed": True, "name": query})
        if action == "browse":
            from owls_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from owls_cli.skills_hub import inspect_skill
