"""Persistent slash-command worker — one OWLSCLI per TUI session.
...
def _run(cli: OWLSCLI, command: str) -> str:
...
def main():
    ...
    os.environ["OWLS_SESSION_KEY"] = args.session_key
    os.environ["OWLS_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = OWLSCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
