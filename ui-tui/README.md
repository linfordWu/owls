...
React + Ink terminal UI for OWLS. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `OWLS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.owls/.owls_history` or under `OWLS_HOME`.
...
```text
ui-tui/
  packages/owls-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      owls-ink.d.ts    type declarations for @owls/ink
