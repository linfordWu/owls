import type { RunExternalProcess } from '@owls/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`owls ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching owls: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`owls ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
