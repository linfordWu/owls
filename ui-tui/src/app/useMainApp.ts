import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@owls/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  // ── Terminal tab title ─────────────────────────────────────────────
  // Show model name + status so users can identify the OWLS tab.
  const shortModel = ui.info?.model?.replace(/^.*\//, '') ?? ''
  const titleStatus = ui.busy ? '⏳' : '✓'
  const terminalTitle = shortModel ? `${titleStatus} ${shortModel} — OWLS` : 'OWLS'
  ...
  const cwd = ui.info?.cwd || process.env.OWLS_CWD || process.cwd()
